# io_utils.py — Excel I/O + helpers + maintenance (centralisé et importable partout)
...
DEFAULT_PLAT = pd.DataFrame(
    {"plateforme": ["Booking", "Airbnb", "Autre"], "couleur": ["#1e90ff", "#e74c3c", "#f59e0b"]}
)
...
def _read_excel_cached(path: str, mtime: float) -> dict:
    ...
    out = {}
    out[SHEET_RES] = pd.read_excel(
        xl,
        sheet_name=SHEET_RES if SHEET_RES in xl.sheet_names else xl.sheet_names[0],
        engine="openpyxl",
        converters={"telephone": normalize_tel},
    ) if xl.sheet_names else pd.DataFrame()
    if SHEET_PLAT in xl.sheet_names:
        out[SHEET_PLAT] = pd.read_excel(xl, sheet_name=SHEET_PLAT, engine="openpyxl")
    else:
        out[SHEET_PLAT] = DEFAULT_PLAT.copy()
    return out
...
def bouton_telecharger(df: pd.DataFrame):
    try:
        ...
        st.sidebar.download_button(
            ...
            help="Télécharge la feuille Reservations uniquement.",
        )
    except Exception as e:
    ...
    if up is not None:
        try:
            ...
            df_pf  = pd.read_excel(xl, sheet_name=SHEET_PLAT, engine="openpyxl") \
                        if SHEET_PLAT in xl.sheet_names else DEFAULT_PLAT.copy()
