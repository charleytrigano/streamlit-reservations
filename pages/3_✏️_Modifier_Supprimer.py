...
if df.empty:
    ...
    # Trier les réservations et réinitialiser l'index pour avoir un identifiant stable
    df_sorted = df.sort_values(by="date_arrivee", ascending=False).reset_index()
    
    # Créer les options pour le selectbox
    options_resa = [f"{idx}: {row['nom_client']} (Arrivée le {row['date_arrivee'].strftime('%d/%m/%Y')})" 
    ...
    if selection_str:
        # Extraire l'index de la sélection
