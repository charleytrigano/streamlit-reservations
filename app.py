# app.py — Villa Tobias (COMPLET, autonome)
# - Réservations / Ajouter / Modifier-Supprimer
# - Plateformes (palette couleurs) sauvegardée dans Excel (feuille "Plateformes")
# - Calendrier mensuel "barres style agenda" (lisible en thème sombre)
# - Rapport (KPI + charts), Liste clients, Export ICS, SMS
# - Restauration XLSX robuste (BytesIO) + openpyxl
# - st.rerun() uniquement (aucun experimental_rerun)
...
def _clean_hex(c: str) -> str:
    ...
    return c if len(c) in (4, 7) else "#999999"
                ...
                if PALETTE_SHEET in xf.sheet_names:
                    ...
                    if {"plateforme","couleur"}.issubset(set(pal_df.columns)):
                        for _, r in pal_df.iterrows():
                            name = str(r["plateforme"]).strip()
                            color = _clean_hex(str(r["couleur"]))
                            if name: palette_new[name] = color
...
def df_to_ics(df: pd.DataFrame, cal_name: str = "Villa Tobias – Réservations") -> str:
    ...
    if df.empty:
        lines = [
            ...
            f"X-WR-CALNAME:{_ics_escape(cal_name)}","CALSCALE:GREGORIAN","METHOD:PUBLISH","END:VCALENDAR"
        ]
        return "\r\n".join(lines) + "\r\n"
...
def sms_message_arrivee(row: pd.Series) -> str:
    ...
    return (
        ...
        f"Téléphone : {tel_aff}\n\n"
        "Bienvenue chez nous !\n\n"
        "Afin d'organiser au mieux votre réception, merci de nous indiquer votre heure d'arrivée.\n"
...
def vue_calendrier(df: pd.DataFrame):
    ...
    core, _ = split_totals(df)
    planning = {date(annee,m,j): [] for j in range(1, calendar.monthrange(annee,m)[1]+1)}
    for _, row in core.iterrows():
    ...
    for semaine in monthcal:
        ...
        for jour in semaine:
            if jour == 0:
                html.append('<td style="background:transparent;"></td>'); continue
            d = date(annee, m, jour)
...
def vue_rapport(df: pd.DataFrame):
    ...
    def bar_chart_metric(title, col):
...
def vue_clients(df: pd.DataFrame):
    ...
    show = data.copy()
    for c in ["date_arrivee","date_depart"]: show[c] = show[c].apply(format_date_str)
    ...
    st.download_button("📥 Télécharger (CSV)", data=show[cols].to_csv(index=False).encode("utf-8"),
...
def vue_export_ics(df: pd.DataFrame):
    ...
    df = ensure_schema(df)
    if df.empty: st.info("Aucune donnée à exporter."); return
    c1, c2, c3 = st.columns(3)
    annees = sorted([int(x) for x in df["AAAA"].dropna().unique()])
    ...
    st.download_button("⬇️ Télécharger reservations.ics", data=ics_text.encode("utf-8"),
                       file_name="reservations.ics", mime="text/calendar")
...
def vue_plateformes():
    ...
    pal = get_palette()
    st.caption("Ajoutez, modifiez, supprimez des plateformes. Cliquez ensuite sur Enregistrer la palette pour les stocker définitivement dans Excel (feuille Plateformes).")
    pf_df = pd.DataFrame([{"plateforme": k, "couleur": v} for k, v in sorted(pal.items())])
    pf_df = st.data_editor(
        pf_df, hide_index=True, use_container_width=True, num_rows="dynamic",
        column_config={
...
def main():
    ...
    try:
        ...
        st.error("Une erreur est survenue dans cette vue :")
        st.exception(e)
