...
def _ser(v, dtype=None) -> pd.Series:
    """Force en Series (évite ndarray sans .fillna/.dropna)."""
    if isinstance(v, pd.Series):
        return v
    return pd.Series(v, dtype=dtype) if v is not None else pd.Series([], dtype=dtype or "object")

def _to_bool_series(s) -> pd.Series:
    s = _ser(s, dtype="object").astype(str).str.strip().str.lower()
    return s.isin(["true","1","oui","vrai","yes","y","t"])

def _to_num(s) -> pd.Series:
    sc = _ser(s, dtype="object").astype(str)\
...
def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["nom_client","plateforme","telephone","email"]:
        df[c] = _ser(df[c], dtype="object").astype(str).replace({"nan": "", "None": ""}).str.strip()
...
def vue_reservations(df, palette):
    ...
    years_ser  = _ser(dfn.get("AAAA"))
    months_ser = _ser(dfn.get("MM"))

    # sécurise types
    years_ser  = pd.to_numeric(years_ser, errors="coerce")
    months_ser = pd.to_numeric(months_ser, errors="coerce")
...
def _copy_block(label: str, payload: str, key: str):
    ...
    st.caption("Sélectionne puis copie (Ctrl/Cmd+C).")
...
def vue_sms(df, palette):
    ...
    pre["sms_envoye"] = _to_bool_series(pre["sms_envoye"]).fillna(False)
    ...
    if pre.empty:
        ...
        if pick:
            ...
            msg = (
                ...
                "Welcome to our home!\n\n"
                "Please let us know your arrival time. Parking available if needed. "
    ...
    post["post_depart_envoye"] = _to_bool_series(post["post_depart_envoye"]).fillna(False)
...
def admin_sidebar(df):
    ...
    if st.sidebar.button("🧹 Purger le cache & recharger"):
        try: st.cache_data.clear()
        except Exception: pass
        try: st.cache_resource.clear()
        except Exception: pass
        st.success("Cache purgé."); st.rerun()
