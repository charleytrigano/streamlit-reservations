# ============================== PARTIE 1/5 — Imports, Config, Styles, Helpers, Appartements, Pays ==============================
# -*- coding: utf-8 -*-
...
# ------------------------------ CONSTANTES ------------------------------
# Chemins par défaut (remplacés selon l’appartement sélectionné)
CSV_RESERVATIONS = "reservations.csv"
CSV_PLATEFORMES  = "plateformes.csv"
APARTMENTS_CSV   = "apartments.csv"
COUNTRIES_CSV    = "countries.csv"    # <— indicatifs / pays / drapeau (éditable)
...
def _apply_custom_css():
    st.markdown(
        """
        <style>
          html, body, [data-testid="stAppViewContainer"] { background:#0f1115; color:#eaeef6; }
          [data-testid="stSidebar"] { background:#151922; border-right:1px solid rgba(124,92,255,.16); }
          .glass {
          ...
          .cal-cell { border:1px solid rgba(124,92,255,.16); border-radius:10px; min-height:110px;
                      padding:8px; position:relative; overflow:hidden; background:#0b0d12; }
          ...
          .print-header { display:none; text-align:center; margin-bottom:8px; }
          .print-header h1 { margin:0; font-size: 20px; }
          /* masquer colonnes techniques dans les dataframes (si classes ajoutées) */
...
DEFAULT_COUNTRIES_ROWS = [
    # prefix,country,flag (emoji)
    ("33","France","🇫🇷"),
    ("34","Espagne","🇪🇸"),
    ("44","Royaume-Uni","🇬🇧"),
    ("49","Allemagne","🇩🇪"),
...
def _ensure_countries_csv():
    if not os.path.exists(COUNTRIES_CSV):
        pd.DataFrame(DEFAULT_COUNTRIES_ROWS, columns=["prefix","country","flag"]).to_csv(
            COUNTRIES_CSV, index=False, encoding="utf-8", lineterminator="\n"
        )

def _read_countries_csv() -> pd.DataFrame:
    _ensure_countries_csv()
    raw = _load_file_bytes(COUNTRIES_CSV)
    df = _detect_delimiter_and_read(raw) if raw else pd.DataFrame()
    if df is None or df.empty:
        return pd.DataFrame(DEFAULT_COUNTRIES_ROWS, columns=["prefix","country","flag"])
    ...
    # dédup plus long préfixe prioritaire
    df = df.drop_duplicates(subset=["prefix"], keep="first")
    return df[["prefix","country","flag"]]

def _save_countries_csv(df: pd.DataFrame):
    ok = df.copy()
    ok.columns = [c.lower() for c in ok.columns]
    ok = ok.rename(columns={"pays":"country","indicatif":"prefix","drapeau":"flag"})
    for c in ("prefix","country","flag"):
        if c not in ok.columns: ok[c] = ""
    ok["prefix"]  = ok["prefix"].astype(str).str.replace(r"\D","", regex=True).str.strip()
    ok["country"] = ok["country"].astype(str).str.strip()
    ok["flag"]    = ok["flag"].astype(str).str.strip()
    ok = ok[(ok["prefix"]!="") & (ok["country"]!="")]
    ok = ok.drop_duplicates(subset=["prefix"], keep="first")
    ok[["prefix","country","flag"]].to_csv(COUNTRIES_CSV, index=False, encoding="utf-8", lineterminator="\n")

def _phone_country(phone: str) -> str:
    """Recherche d’abord dans countries.csv (préfixes les plus longs), sinon fallback table par défaut."""
    p = str(phone or "").strip()
    if not p: return ""
    if p.startswith("0"):  # local FR
        return "France"
    # strip + ou 00
    ...
    # lookup CSV
    ctab = _read_countries_csv()
    for pref in sorted(ctab["prefix"].unique(), key=lambda x: -len(x)):
        if p1.startswith(pref):
            row = ctab[ctab["prefix"]==pref].iloc[0]
            return str(row["country"])
    # fallback par défaut
    for pref in sorted(DEFAULT_PREFIX.keys(), key=lambda x: -len(x)):
        if p1.startswith(pref): return DEFAULT_PREFIX[pref]
    return "Inconnu"
...
def charger_donnees(csv_reservations: str, csv_plateformes: str):
    ...
    if rawp is not None:
        try:
            ...
            if {"plateforme","couleur"}.issubset({c.lower() for c in pal_df.columns}):
                # normalise
                cols = {c.lower(): c for c in pal_df.columns}
                pcol, ccol = cols["plateforme"], cols["couleur"]
                palette = {str(r[pcol]).strip(): str(r[ccol]).strip() for _, r in pal_df.iterrows()}
        except Exception as e:
            st.warning(f"Erreur de palette : {e}")
    return df, palette

# ------------------------------ COULEURS/PAL PALETTE helpers ------------------------------
def _to_hex_or_default(x, default="#666666"):
    s = str(x or "").strip()
    if re.fullmatch(r"#([0-9A-Fa-f]{6})", s):
        return s
    return default

def _coerce_palette_to_dict(pal):
    if isinstance(pal, dict):
        return {str(k): _to_hex_or_default(v) for k, v in pal.items()}
    try:
        if hasattr(pal, "to_dict") and hasattr(pal, "columns"):
            cols = {c.lower(): c for c in pal.columns}
            if "plateforme" in cols and "couleur" in cols:
                return {str(r[cols["plateforme"]]).strip(): _to_hex_or_default(r[cols["couleur"]]) for _, r in pal.iterrows()}
    except Exception:
        pass
...
# ============================== PARTIE 2/5 — Vues Accueil, Réservations, Ajout, Modification ==============================
...
# ============================== PARTIE 3/5 — Plateformes, Calendrier, Rapport ==============================
...
def vue_calendrier(df: pd.DataFrame, palette: dict):
    # ATTENTION : nécessite from html import escape en PARTIE 1
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_rapport(df: pd.DataFrame, palette: dict):
    ...
    def jours_dans_mois(periode_str):
        annee, mois_ = map(int, periode_str.split("-"))
        return monthrange(annee, mois_)[1]
...
from urllib.parse import quote  # (déjà importé en PARTIE 1 normalement)
...
def vue_sms(df: pd.DataFrame, palette: dict):
    ...
    if pre.empty:
        ...
        if pick:
            ...
            dep_str = r["date_depart"].strftime("%d/%m/%Y") if pd.notna(r["date_depart"]) else ""

            msg = (
                f"{apt_name.upper()}\n"
                f"Plateforme : {r.get('plateforme', 'N/A')}\n"
                f"Arrivée : {r['date_arrivee'].strftime('%d/%m/%Y')}  Départ : {dep_str}  Nuitées : {nuitees_i}\n\n"
                ...
                "we kindly ask you to fill out the form you will find via the link below:\n"
                f"{link_form}\n\n"
...
def vue_settings(df: pd.DataFrame, palette: dict):
    """Page Paramètres : sauvegarde/restauration, vider cache, écraser apartments.csv."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    st.header("## ⚙️ Paramètres")
    st.subheader(apt_name)
    print_buttons()
    st.caption("Sauvegarde, restauration, cache et écrasement de apartments.csv.")
