...
# ============================== PART 2/5 — NORMALISATION, SAUVEGARDE, APARTMENTS ==============================

def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    """Normalise le DataFrame pour garantir la présence/typage des colonnes attendues."""
...
def sauvegarder_donnees(df: pd.DataFrame) -> bool:
    """Sauvegarde le CSV de reservations pour l’appartement courant."""
    try:
...
def charger_donnees(csv_reservations: str, csv_plateformes: str):
    """Charge données + palette. Crée des fichiers si absents."""
...
def _load_data_for_active_apartment():
    """Wrapper sûr autour de charger_donnees pour l’appartement actif."""
...
def vue_accueil(df: pd.DataFrame, palette: dict):
    """Dashboard du jour : arrivées, départs, J+1."""
...
def vue_modifier(df: pd.DataFrame, palette: dict):
    """Édition et suppression d’une réservation."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_plateformes(df: pd.DataFrame, palette: dict):
    ...
    # Détecte si ColorColumn est disponible (Streamlit récent)
    HAS_COLORCOL = hasattr(getattr(st, "column_config", object), "ColorColumn")

    # Liste des plateformes vues dans les données
    plats_df = sorted(
    ...
    # Aperçu visuel si ColorColumn indisponible
    ...
    if c1.button("💾 Enregistrer la palette", key="save_palette_btn"):
        try:
            ...
            # Sauvegarde dans le fichier spécifique à l'appartement courant
...
def vue_calendrier(df: pd.DataFrame, palette: dict):
    """Grille mensuelle + détails du mois sélectionné."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_rapport(df: pd.DataFrame, palette: dict):
    """KPI + occupation + comparatifs + analyse pays + courbes d’évolution."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    ...
    # Déduire pays si manquant, à partir du téléphone / indicatif
...
def vue_sms(df: pd.DataFrame, palette: dict):
    ...
    if pre.empty:
        ...
        if pick:
            ...
            # Marquer envoyé
            if st.button("✅ Marquer 'SMS envoyé' pour ce client", key=f"sms_sent_{sel_idx}"):
                try:
    ...
    if post.empty:
        ...
        if pick2:
            ...
            if st.button("✅ Marquer 'post-départ envoyé' pour ce client", key=f"post_sent_{sel_idx2}"):
                try:
