# app.py — Réservations Villa Tobias (avec logo/icône en haut + sidebar)
...
LOGO_PATH = "logo.png"               # <-- place ton image ici (à la racine du repo)
...
def _read_logo_b64(path: str) -> str | None:
    p = Path(path)
    if not p.exists():
        return None
    try:
...
def render_logo(title="VILLA TOBIAS", subtitle="Réservations & planning"):
    b64 = _read_logo_b64(LOGO_PATH)
    if not b64:
        st.markdown(f"<h2 style='margin:0'>{title}</h2><div style='opacity:.7'>{subtitle}</div><hr>", unsafe_allow_html=True)
        return
    st.markdown(
        f"""
        <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.5rem">
          <img src="data:image/png;base64,{b64}"
               style="width:48px;height:48px;border-radius:50%;object-fit:cover;border:1px solid rgba(0,0,0,.1)" />
          <div>
            <div style="font-weight:700;font-size:20px;line-height:1.1"> {title} </div>
            <div style="opacity:.7;font-size:13px">{subtitle}</div>
          </div>
        </div>
        <hr style="opacity:.25;margin:.5rem 0 1rem 0">
        """,
        unsafe_allow_html=True
    )

def render_logo_sidebar(caption="Bienvenue 👋"):
    b64 = _read_logo_b64(LOGO_PATH)
    if not b64:
        return
    st.sidebar.markdown(
        ...
        <div style="display:flex;align-items:center;gap:.5rem;margin:.25rem 0 .75rem 0">
          <img src="data:image/png;base64,{b64}"
               style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:1px solid rgba(0,0,0,.12)" />
          <div style="font-weight:600">VILLA TOBIAS</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
# ====== TOTAUX (st.metric) — fiable mobile & thème sombre ==================
...
def metrics_bar(df: pd.DataFrame, prefix: str = ""):
    ...
    def lbl(name): return (f"Total {name}" if prefix else name)
    c1, c2, c3, c4, c5 = st.columns(5)
    with c1: st.metric(lbl("brut"), f"{brut:,.2f} €".replace(",", " "), help=f"{brut_nuit:,.2f} €/nuit".replace(",", " "))
    with c2: st.metric(lbl("net"),  f"{net:,.2f} €".replace(",", " "),  help=f"{net_nuit:,.2f} €/nuit".replace(",", " "))
    with c3: st.metric(lbl("charges"), f"{ch:,.2f} €".replace(",", " "))
    with c4: st.metric(lbl("nuitées"), f"{int(nts)}")
    with c5: st.metric("Commission moyenne", f"{pct_mean:.2f} %")
...
def ensure_schema(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["date_arrivee", "date_depart"]:
        if col in df.columns: df[col] = df[col].apply(to_date_only)
    for col in ["prix_brut", "prix_net", "charges", "%"]:
        if col in df.columns: df[col] = pd.to_numeric(df[col], errors="coerce")
    if "prix_brut" in df.columns and "prix_net" in df.columns:
        if "charges" not in df.columns: df["charges"] = df["prix_brut"] - df["prix_net"]
    ...
    if "ical_uid" not in df.columns: df["ical_uid"] = ""
...
def _marque_totaux(df: pd.DataFrame) -> pd.Series:
    ...
    for c in ["date_arrivee","date_depart"]:
        if c in df.columns: has_no_dates &= df[c].isna()
    has_money = pd.Series(False, index=df.index)
    for c in ["prix_brut","prix_net","charges"]:
        if c in df.columns: has_money |= df[c].notna()
...
def sauvegarder_donnees(df: pd.DataFrame):
    ...
    try:
        ...
        if ok: st.info("☁️ Sauvegarde GitHub OK.")
        else:  st.caption(f"ℹ️ GitHub : {msg}")
    except Exception as e:
...
def vue_reservations(df: pd.DataFrame):
    ...
    with c_btns:
        ...
        st.download_button("💾 Sauvegarde XLSX", data=data_xlsx,
        ...
        if st.session_state.get("show_restore_top"):
            ...
            if up is not None: restauration_from_uploader(up)
    ...
    for col in ["date_arrivee","date_depart"]:
        if col in show.columns: show[col] = show[col].apply(format_date_str)
...
def vue_ajouter(df: pd.DataFrame):
    ...
    with st.form("ajout_resa"):
        c1, c2 = st.columns(2)
        with c1: nom = inline_input("Nom", st.text_input, key="add_nom", value="")
        with c2: tel = inline_input("Téléphone (+33...)", st.text_input, key="add_tel", value="")
        ...
        with c6:
            prix_brut = inline_input("Prix brut (€)", st.number_input, key="add_brut",
                                     min_value=0.0, step=1.0, format="%.2f") or 0.0
        with c7:
            prix_net = inline_input("Prix net (€)", st.number_input, key="add_net",
                                    min_value=0.0, step=1.0, format="%.2f") or 0.0
    ...
    if ok:
        if prix_net > prix_brut:
            st.error("Le prix net ne peut pas être supérieur au prix brut."); return
        if depart < arrivee + timedelta(days=1):
            st.error("La date de départ doit être au moins le lendemain de l’arrivée."); return
        ligne = {
...
def vue_calendrier(df: pd.DataFrame):
    ...
    if not annees: st.warning("Aucune année disponible."); return
    with c_annee:
...
def vue_rapport(df: pd.DataFrame):
    ...
    if not annees: st.info("Aucune année disponible."); return
    cA, cB, cC = st.columns([1, 2, 1])
    with cA: annee = st.selectbox("Année", annees, index=len(annees) - 1, key="rapport_annee")
    data = df[df["AAAA"] == int(annee)].copy()
    plateformes = ["Toutes"] + sorted(data["plateforme"].dropna().unique().tolist())
    with cB: filtre_plateforme = st.selectbox("Plateforme", plateformes, key="rapport_pf")
    with cC: filtre_mois_label = st.selectbox("Mois (01–12)", ["Tous"] + [f"{i:02d}" for i in range(1, 13)], key="rapport_mois")
    if filtre_plateforme != "Toutes": data = data[data["plateforme"] == filtre_plateforme]
    if filtre_mois_label != "Tous":   data = data[data["MM"] == int(filtre_mois_label)]
    ...
    if stats.empty: st.info("Aucune donnée après agrégation."); return
...
def vue_clients(df: pd.DataFrame):
    ...
    annees = sorted([int(x) for x in df["AAAA"].dropna().unique()])
    with cA: annee = st.selectbox("Année", annees) if annees else None
    with cB: mois  = st.selectbox("Mois", ["Tous"] + list(range(1,13)))
    data = df.copy()
    if annee: data = data[data["AAAA"] == annee]
    if mois != "Tous": data = data[data["MM"] == int(mois)]
    if data.empty: st.info("Aucune donnée pour cette période."); return
    with pd.option_context('mode.use_inf_as_na', True):
    ...
    show = data.copy()
    for c in ["date_arrivee","date_depart"]:
        if c in show.columns: show[c] = show[c].apply(format_date_str)
    st.dataframe(show[cols], use_container_width=True)
    st.download_button("📥 Télécharger la liste (CSV)",
                       data=show[cols].to_csv(index=False).encode("utf-8"),
                       file_name="liste_clients.csv", mime="text/csv")
...
def vue_sms(df: pd.DataFrame):
    ...
    if dft.empty: st.info("Aucune donnée."); return
    today = date.today()
    cA, cB = st.columns(2)
    with cA: cible_arrivee = st.date_input("📅 Date d’arrivée ciblée (accueil)", value=today + timedelta(days=1))
    with cB: cible_depart  = st.date_input("📅 Date de départ ciblée (remerciement J+1)", value=today - timedelta(days=1))
    st.markdown("#### 🟢 Arrivées (messages d’accueil)")
    arr = dft[(dft["date_arrivee"] == cible_arrivee)].copy()
    if arr.empty: st.info("Aucune arrivée à cette date.")
    else:
        for _, row in arr.iterrows():
            ...
            with col2:
                if tel_ui: st.markdown(f"[📞 Appeler]({tel_ui})")
            with col3:
                if sms_uri: st.markdown(f"[📲 SMS]({sms_uri})")
            st.code(sms_txt, language="text")
    st.markdown("#### 🟡 Départs (remerciement J+1)")
    dep = dft[(dft["date_depart"] == cible_depart)].copy()
    if dep.empty: st.info("Aucun départ à cette date.")
    else:
    ...
    for block in re.split(r"BEGIN:VEVENT", text)[1:]:
        ...
        def parse_dt(s):
            ...
            try:
                ...
                return None
            return None
        d1 = parse_dt(dtstart); d2 = parse_dt(dtend)
...
def vue_ical(df: pd.DataFrame):
    ...
    if sources:
        for i, s in enumerate(sources):
            ...
            with c1: name = st.text_input(f"Nom #{i+1}", value=s.get("name",""), key=f"ical_name_{i}")
            with c2: url  = st.text_input(f"URL #{i+1}", value=s.get("url",""), key=f"ical_url_{i}")
            with c3:
                if st.button("🗑 Supprimer", key=f"del_src_{i}"):
                    sources.pop(i); save_ical_sources(sources); st.rerun()
    ...
    with c1: new_name = st.text_input("Nom", key="new_ical_name", placeholder="Booking / Airbnb / Autre")
    with c2: new_url  = st.text_input("URL", key="new_ical_url", placeholder="https://...ics")
    with c3:
        if st.button("Ajouter"):
            if new_name and new_url:
                sources.append({"name": new_name.strip(), "url": new_url.strip()})
                save_ical_sources(sources); st.rerun()
            else:
                st.warning("Veuillez renseigner un nom et une URL.")
    st.markdown("#### ⬇️ Charger et comparer")
    if st.button("Analyser les sources iCal"):
        if not sources:
            st.warning("Ajoutez au moins une source."); return
        ...
        a_importer, erreurs = [], []
        for s in sources:
            nom_src = s.get("name",""); url = s.get("url","")
            try:
                r = requests.get(url, timeout=20)
                if r.status_code != 200:
                    erreurs.append(f"{nom_src}: HTTP {r.status_code}"); continue
                evs = parse_ics(r.text)
                for ev in evs:
                    if ev["uid"] and ev["uid"] in uids_existants: continue
        ...
        if erreurs: st.warning("Erreurs rencontrées :\n- " + "\n- ".join(erreurs))
        if not a_importer:
            st.success("Aucun nouvel événement à importer (tout est déjà présent)."); return
...
def main():
    ...
    # Option de restauration en sidebar
    with st.sidebar:
        st.markdown("### ")
        holder = st.container()
        with holder:
