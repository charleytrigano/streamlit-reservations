...
# - Palette plateformes éditable (session)
# - Restauration XLSX en mémoire (BytesIO) + Sauvegarde XLSX (openpyxl)
# - Réservations / Ajouter / Modifier / Calendrier (cases colorées + noms) / Rapport / Clients / ICS / SMS
# - Thème sombre friendly (texte auto contrasté)
# - Zéro expander imbriqué, pas d’API expérimentale
...
def ensure_schema(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["paye"] = df.get("paye", False)
    df["sms_envoye"] = df.get("sms_envoye", False)
...
def bouton_restaurer():
    up = st.sidebar.file_uploader("📤 Restauration (sélectionne ton .xlsx)", type=["xlsx"])
    if up is not None:
        try:
            raw = up.read()
            if not raw:
    ...
    for semaine in monthcal:
        ...
        for jour in semaine:
            if jour == 0:
                ...
                # texte = date + noms
                if len(items) > 5:
                    lines = [str(jour)] + [f"{nom}" for _, nom in items[:5]] + [f"... (+{len(items)-5})"]
                else:
                    lines = [str(jour)] + [f"{nom}" for _, nom in items]
                row_text.append("\n".join(lines))
...
def vue_rapport(df: pd.DataFrame):
    ...
    def bar_chart_metric(label_, col_):
        if stats.empty:
            return
        pvt = stats.pivot(index="MM", columns="plateforme", values=col_).fillna(0).sort_index()
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        pd.DataFrame(detail[cols_detail]).to_excel(writer, index=False)
    st.download_button(
