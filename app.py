# PART 1/5 - IMPORTS, CONFIG, CONSTANTS, STYLE, HELPERS, INDICATIFS, APARTMENTS
...
# CONSTANTS
# -----------------------------------------------------------------------------
# Chemins par defaut; seront remplaces des que l'appartement actif est choisi
CSV_RESERVATIONS = "reservations.csv"
CSV_PLATEFORMES = "plateformes.csv"
APARTMENTS_CSV = "apartments.csv"

# Table des indicatifs pays editable par l'utilisateur
COUNTRIES_CSV = "countries_with_flags.csv"
...
# STYLE
# -----------------------------------------------------------------------------
def apply_style(light: bool) -> None:
    bg = "#fafafa" if light else "#0f1115"
    fg = "#0f172a" if light else "#eaeef6"
    side = "#f2f2f2" if light else "#171923"
    border = "rgba(17,24,39,.08)" if light else "rgba(124,92,255,.16)"
    pill_bg = "#eee" if light else "#2a2f3a"
    pill_fg = "#222" if light else "#eee"
...
.chip {{
  ...
  font-size:.86rem; background:{pill_bg}; color:{pill_fg};
}}
.cal-grid {{
  display:grid; grid-template-columns: repeat(7, 1fr); gap:8px;
}}
.cal-cell {{
  border:1px solid {border}; border-radius:10px; min-height:110px; padding:8px; position:relative; overflow:hidden; background:{cell_bg};
}}
...
/* Impression A4 paysage, cache les colonnes techniques */
@page {{
  size: A4 landscape;
  margin: 10mm;
}}
...
def print_buttons(location: str = "main") -> None:
    target = st.sidebar if location == "sidebar" else st
    target.button("Imprimer", key=f"print_btn_{location}")
    st.markdown(
        """
<script>
(function() {
  const labels = Array.from(parent.document.querySelectorAll('button span, button p'));
  const node = labels.find(n => n.textContent && n.textContent.trim() === 'Imprimer');
...
# -----------------------------------------------------------------------------
# INDICATIFS PAYS (CSV editable)
# -----------------------------------------------------------------------------
def create_indicatifs_csv() -> None:
    """Cree un CSV d'indicatifs si absent (UTF-8)."""
    if os.path.exists(COUNTRIES_CSV):
        return
    rows = [
        # code, country, flag (emoji)
        ("33", "France", "🇫🇷"),
        ("34", "Espagne", "🇪🇸"),
        ("39", "Italie", "🇮🇹"),
        ("41", "Suisse", "🇨🇭"),
    ...
    df = pd.DataFrame(rows, columns=["code", "country", "flag"])
    df.to_csv(COUNTRIES_CSV, index=False, encoding="utf-8", lineterminator="\n")
...
def load_indicatifs() -> pd.DataFrame:
    ...
    try:
        ...
        df = pd.DataFrame(columns=["code", "country", "flag"])
    ...
    df = df[(df["code"] != "") & (df["country"] != "")]
    # dedup on longest code first
    df = df.drop_duplicates(subset=["code"], keep="first")
    return df[["code", "country", "flag"]]

def _phone_country(phone: str) -> str:
    """Retourne le pays d'apres l'indicatif dans COUNTRIES_CSV. Par defaut France si '0...'."""
    ...
    indi = load_indicatifs()
    # chercher le match le plus long
    best = None
    best_len = 0
    for code in indi["code"].astype(str):
        if p1.startswith(code) and len(code) > best_len:
            best = code
            best_len = len(code)
    if best is None:
        return "Inconnu"
    row = indi.loc[indi["code"] == best].iloc[0]
    return str(row["country"]) or "Inconnu"
...
# PART 2/5 - VUES: ACCUEIL, RESERVATIONS, AJOUTER, MODIFIER, PLATEFORMES
...
def vue_accueil(df: pd.DataFrame, palette: dict):
    ...
    with c1:
        st.subheader("Arrivees du jour")
        st.dataframe(arr if not arr.empty else pd.DataFrame({"info": ["Aucune arrivee."]}), use_container_width=True)
    with c2:
        st.subheader("Departs du jour")
        st.dataframe(dep if not dep.empty else pd.DataFrame({"info": ["Aucun depart."]}), use_container_width=True)
    with c3:
        st.subheader("Arrivees J+1 (demain)")
        st.dataframe(arr_plus1 if not arr_plus1.empty else pd.DataFrame({"info": ["Aucune arrivee demain."]}), use_container_width=True)


def vue_reservations(df: pd.DataFrame, palette: dict):
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    st.header(f"Reservations - {apt_name}")
    print_buttons()
    if df is None or df.empty:
        st.info("Aucune reservation.")
        return
    ...
    year = c1.selectbox("Annee", ["Toutes"] + years_avail, index=0)
    month = c2.selectbox("Mois", ["Tous"] + months_avail, index=0)
    plat = c3.selectbox("Plateforme", ["Toutes"] + plats_avail, index=0)
    payf = c4.selectbox("Paiement", ["Tous", "Paye uniquement", "Non paye uniquement"], index=0)
    ...
    if payf == "Paye uniquement":
        data = data[_to_bool_series(data["paye"]) == True]
    elif payf == "Non paye uniquement":
...
<div class='glass'>
  ...
  <span class='chip'><small>Nuitees</small><br><strong>{nuits}</strong></span>
  <span class='chip'><small>ADR (net)</small><br><strong>{adr:,.2f} €</strong></span>
    ...
    # Masquer colonnes techniques a l'affichage
    hide_cols = ["res_id", "ical_uid", "%", "charges", "base"]
    show_cols = [c for c in data.columns if c not in hide_cols]
    st.dataframe(data[show_cols], use_container_width=True)


def vue_ajouter(df: pd.DataFrame, palette: dict):
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    st.header(f"Ajouter une reservation - {apt_name}")
    print_buttons()
    ...
    with st.form("form_add", clear_on_submit=True):
        ...
        with c1:
            ...
            arr = st.date_input("Arrivee", date.today())
            dep = st.date_input("Depart", date.today() + timedelta(days=1))
        with c2:
            ...
            menage = st.number_input("Menage (€)", min_value=0.0, step=0.01)
            taxes = st.number_input("Taxes sejour (€)", min_value=0.0, step=0.01)
            paye = st.checkbox("Paye", value=False)
        ...
        if st.form_submit_button("Ajouter"):
            if not nom or dep <= arr:
                ...
                if sauvegarder_donnees(df2):
                    st.success(f"Reservation pour {nom} ajoutee.")
                    st.rerun()
...
def vue_modifier(df: pd.DataFrame, palette: dict):
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    st.header(f"Modifier / Supprimer - {apt_name}")
    print_buttons()
    if df.empty:
        st.info("Aucune reservation.")
        return
    ...
    sel = st.selectbox("Selectionnez une reservation", options=options, index=None)
    if not sel:
        return
    ...
    with st.form(f"form_mod_{original_idx}"):
        ...
        with c1:
            ...
            tel = st.text_input("Telephone", value=row.get("telephone", "") or "")
            arrivee = st.date_input("Arrivee", value=row.get("date_arrivee"))
            depart = st.date_input("Depart", value=row.get("date_depart"))
        with c2:
            palette_keys = list(palette.keys())
            plat_idx = palette_keys.index(row.get("plateforme")) if row.get("plateforme") in palette_keys else 0
            plat = st.selectbox("Plateforme", options=palette_keys, index=plat_idx)
            paye = st.checkbox("Paye", value=bool(row.get("paye", False)))
            ...
            menage = st.number_input("Menage", min_value=0.0, step=0.01, value=menage)
            taxes = st.number_input("Taxes sejour", min_value=0.0, step=0.01, value=taxes)
        ...
        if b2.form_submit_button("Supprimer"):
...
def vue_plateformes(df: pd.DataFrame, palette: dict):
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    st.header(f"Plateformes et couleurs - {apt_name}")
    print_buttons()
    ...
    if not has_colorcol and not edited.empty:
        ...
        for _, r in edited.iterrows():
            ...
            chips.append(
                "<span style='display:inline-block;margin:4px 6px;padding:6px 10px;border-radius:12px;"
                f"background:{col if ok else '#666'};color:#fff;'>{plat} {col}</span>"
            )
        if chips:
    ...
    if c1.button("Enregistrer la palette", key="save_palette_btn"):
        try:
            ...
            st.success("Palette enregistree")
            st.cache_data.clear()
            st.rerun()
        except Exception as e:
            st.error(f"Erreur : {e}")

    if c2.button("Palette par defaut", key="restore_palette_btn"):
        try:
            target = st.session_state.get("CSV_PLATEFORMES", CSV_PLATEFORMES)
            pd.DataFrame(list(DEFAULT_PALETTE.items()), columns=["plateforme", "couleur"]).to_csv(
                target, sep=";", index=False, encoding="utf-8", lineterminator="\n"
            )
            st.success("Palette restauree")
            st.cache_data.clear()
            st.rerun()
        except Exception as e:
            st.error(f"Erreur : {e}")
...
# PART 3/5 - VUES: CALENDRIER & RAPPORT

def vue_calendrier(df: pd.DataFrame, palette: dict):
    """Grille mensuelle simple + liste des réservations du mois sélectionné."""
    ...
    # Helper pour les réservations actives un jour donné
    def day_resas(d):
    ...
    if plat_sel != "Toutes":
        rows = rows[rows["plateforme"].astype(str) == plat_sel]
    ...
    show_cols = ["nom_client", "plateforme", "date_arrivee", "date_depart", "nuitees", "paye", "pays"]
...
def vue_rapport(df: pd.DataFrame, palette: dict):
    """Rapport de performance: occupation, comparaisons, finances, pays."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    ...
    # Pays (déduction par téléphone si manquant)
    dfa["_pays"] = dfa["pays"].astype(str).replace({"": np.nan, "nan": np.nan})
    ...
    st.markdown(
        (
            ...
            f"<span class='chip'><small>Nuitées occupées</small><br><strong>{int(filtered_nuitees)}</strong></span>"
            f"<span class='chip'><small>Jours dispos</small><br><strong>{int(filtered_jours)}</strong></span>"
            f"<span class='chip'><small>Pays filtré</small><br><strong>{payf if payf!='Tous' else 'Tous'}</strong></span>"
            "</div>"
        ),
        unsafe_allow_html=True,
    )
    ...
    def jours_dans_annee(an: int) -> int:
    ...
    if not occ_annee.empty:
        default_years = sorted(occ_annee["annee"].unique())[-2:] if occ_annee["annee"].nunique() >= 2 else sorted(
            occ_annee["annee"].unique()
        )
    ...
    agg_pays["part_revenu_%"] = np.where(
        total_net > 0, (pd.to_numeric(agg_pays["prix_net"], errors="coerce").fillna(0) / total_net) * 100, 0.0
    )
    ...
    st.markdown(
        (
            "<div class='glass'>"
            f"<span class='chip'><small>Année</small><br><strong>{year_pays}</strong></span>"
            f"<span class='chip'><small>Pays distincts</small><br><strong>{nb_pays}</strong></span>"
            f"<span class='chip'><small>Total réservations</small><br><strong>{total_res}</strong></span>"
    ...
    st.dataframe(disp[order_cols], use_container_width=True)
...
# PART 4/5 - VUES: SMS, EXPORT ICS, GOOGLE SHEET, CLIENTS, ID, INDICATIFS

# ---------------------- SMS ----------------------

def vue_sms(df: pd.DataFrame, palette: dict):
    """Messages SMS/WhatsApp avant arrivée et après départ (copier/coller + marquage)."""
    from urllib.parse import quote

    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    st.header(f"SMS — {apt_name}")
    print_buttons()
    ...
    if pre.empty:
        ...
        if pick:
            ...
            # Construire le message proprement pour éviter tout problème d'échappement
            parts_fr = [
                f"{apt_name.upper()}",
                ...
                "merci de remplir la fiche via le lien suivant :",
                f"{link_form}",
                "",
                "Un parking est à votre disposition sur place.",
                "",
                "Check-in à partir de 14:00, check-out avant 11:00. Nous serons présents pour vous remettre les clés.",
                "",
            ...
            parts_en = [
                ...
                "",
                "We are delighted to welcome you soon to Nice. To help us organize your arrival, please fill in this form:",
                f"{link_form}",
                "",
                "Parking is available on site.",
                "",
                "Check-in from 2:00 p.m., check-out before 11:00 a.m. We'll hand you the keys on arrival.",
                "",
...
def vue_export_ics(df: pd.DataFrame, palette: dict):
    """Génère un fichier .ics (événements journée entière) à importer dans Google Calendar."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_clients(df: pd.DataFrame, palette: dict):
    ...
    clients = clients[[c for c in cols_order if c in clients.columns]]
...
def _ensure_indicatifs_file():
    ...
    base = pd.DataFrame(
        [
            {"dial_code": "33", "country": "France", "flag": "🇫🇷"},
            {"dial_code": "34", "country": "Espagne", "flag": "🇪🇸"},
            {"dial_code": "39", "country": "Italie", "flag": "🇮🇹"},
            {"dial_code": "41", "country": "Suisse", "flag": "🇨🇭"},
            {"dial_code": "32", "country": "Belgique", "flag": "🇧🇪"},
        ]
    )
...
def load_indicatifs() -> pd.DataFrame:
    ...
    try:
        ...
        df = pd.DataFrame(columns=["dial_code", "country", "flag"])
    for c in ["dial_code", "country", "flag"]:
        if c not in df.columns:
            df[c] = ""
    # normalisation minimale
    df["dial_code"] = df["dial_code"].astype(str).str.replace(r"\D", "", regex=True).str.strip()
    df["country"] = df["country"].astype(str).str.strip()
    df["flag"] = df["flag"].astype(str).str.strip()
...
def vue_indicatifs(df: pd.DataFrame, palette: dict):
    ...
    st.caption("Astuce : le drapeau peut être un emoji (ex: 🇫🇷) ou laissé vide.")

    col_cfg = {
        "dial_code": st.column_config.TextColumn("Indicatif (sans +)", help="Ex: 33 pour France"),
    ...
    if c1.button("💾 Enregistrer les indicatifs"):
        try:
            to_save = edited.copy()
            to_save["dial_code"] = to_save["dial_code"].astype(str).str.replace(r"\D", "", regex=True).str.strip()
            to_save["country"] = to_save["country"].astype(str).str.strip()
            to_save["flag"] = to_save["flag"].astype(str).str.strip()
            to_save = to_save[to_save["dial_code"] != ""].drop_duplicates(subset=["dial_code"], keep="first")
...
def vue_settings(df: pd.DataFrame, palette: dict):
    """Sauvegarde/restauration des données + cache + outil secours apartments.csv."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    st.header("Paramètres")
    st.subheader(apt_name)
    print_buttons()
    st.caption("Sauvegarde, restauration, cache et écrasement de apartments.csv si besoin.")
    ...
    # Restauration
    st.markdown("### Restauration (remplacer les données)")
    ...
    # Cache
    st.markdown("### Cache")
    if st.button("Vider le cache et recharger", key="clear_cache_btn_settings"):
        try:
            st.cache_data.clear()
        except Exception:
            pass
    ...
    st.markdown("### Outil secours apartments.csv")
...
def main():
    ...
    # Thème (applique le CSS défini en PART 1)
    try:
    ...
    pages = {
        ...
        "👥 Clients": vue_clients,
        "🆔 ID": vue_id,
        "🌍 Indicateurs pays": vue_indicatifs,  # édition/consultation des indicatifs
