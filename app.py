...
def apply_style(light: bool):
    ...
    st.markdown(
        ...
        <style>
          ...
          /* Impression : A4 paysage, masque la chrome UI */
          ...
          @media print {{
            ...
            [data-testid="stDataFrame"] th, [data-testid="stDataFrame"] td {{ padding: 4px 6px !important; }}
          }}
        </style>
        """,
...
# ---------- Helpers impression & noms de fichiers actifs ----------
def _apply_custom_css():
    """(Déjà injecté via apply_style, laissé pour compat future)"""
    pass

def render_print_header(title: str | None = None):
    """En-tête spécial impression (n’apparaît qu’au print)."""
...
def _get_active_paths():
    """Retourne (csv_reservations, csv_plateformes) actifs, robustes."""
...
def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    """Garantit la présence des colonnes et types attendus, recalcule prix_net/base/%/nuitees."""
    if df_in is None or len(df_in) == 0:
        return pd.DataFrame(columns=BASE_COLS)

    df = df_in.copy()
    df.columns = df.columns.astype(str).str.strip()
    df.rename(columns=BASE_RENAMES, inplace=True)
...
def sauvegarder_donnees(df: pd.DataFrame) -> bool:
    """Sauvegarde le CSV des réservations actives (avec dates formatées), vide le cache."""
    try:
...
# ============================== CHARGEMENT AVEC PALETTE ==============================
@st.cache_data(show_spinner=False)
def charger_donnees(csv_reservations: str, csv_plateformes: str):
    """Charge df & palette depuis les fichiers fournis, en créant des fichiers vides au besoin."""
...
def _read_apartments_csv() -> pd.DataFrame:
    """Lit apartments.csv et normalise les colonnes {slug,name} (séparateur auto)."""
    try:
...
def _load_data_for_active_apartment():
    """Charge (df, palette) à partir des chemins a

ctifs de la session (fallback robuste)."""
    csv_res = st.session_state.get("CSV_RESERVATIONS", CSV_RESERVATIONS)
    csv_pal = st.session_state.get("CSV_PLATEFORMES",  CSV_PLATEFORMES)
    try:
...
# ============================== VUES — ACCUEIL / RÉSERVATIONS / AJOUT / MODIF ==============================
...
def vue_reservations(df, palette):
    """Liste + KPIs + filtres (année, mois, plateforme, paiement)."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
    ...
    # masque colonnes techniques (pour l'affichage)
    cols_hide = ["ical_uid", "%"]
    show_cols = [c for c in data.columns if c not in cols_hide]
...
# ============================== VUES — PLATEFORMES / CALENDRIER / RAPPORT ==============================
def vue_plateformes(df, palette):
    """Gestion des plateformes et couleurs (palette enregistrée par appartement)."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_calendrier(df, palette):
    """Grille mensuelle + détail et KPI du mois choisi."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_rapport(df, palette):
    """KPI Occupation + comparaisons + finances + pays + évolution mensuelle du taux d’occupation."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
# ============================== VUES — SMS / ICS / GOOGLE SHEET / CLIENTS / ID / PARAMÈTRES ==============================
...
def _google_form_prefill(res_id, nom, phone, arr, dep):
    # On conserve le lien court fourni (fiable et simple).
...
def vue_sms(df, palette):
    ...
    if pre.empty:
        ...
        if pick:
            ...
            d_arr = r["date_arrivee"].strftime("%d/%m/%Y")
            d_dep = r["date_depart"].strftime("%d/%m/%Y") if pd.notna(r["date_depart"]) else ""

            msg = (
                f"{apt_name.upper()}\n"
                f"Plateforme : {r.get('plateforme', 'N/A')}\n"
                f"Arrivée : {d_arr}  Départ : {d_dep}  Nuitées : {nuitees}\n\n"
...
def vue_export_ics(df, palette):
    """Export du calendrier au format .ics (par année, et filtre plateforme)."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_google_sheet(df, palette):
    """Intégration Google Form + Google Sheet (réponses publiées)."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_clients(df, palette):
    """Liste propre des clients (dédoublonnée)."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
def vue_id(df, palette):
    """Table des identifiants (res_id / ical_uid)."""
    apt = _current_apartment()
    apt_name = apt["name"] if apt else "—"
...
# ============================== PARAMÈTRES (Sauvegarde / Restauration / Cache / Outil CSV apts) ==============================
def vue_settings(df: pd.DataFrame, palette: dict):
    ...
    # ---- Restauration (remplacement du fichier courant) ----
...
def _apply_print_css():
    st.markdown(
        ...
        @page { size: A4 landscape; margin: 10mm; }
        @media print {
          /* cacher header/sidebar/toolbars */
          [data-testid="stHeader"], [data-testid="stToolbar"], [data-testid="stSidebar"] { display: none !important; }
          .print-hide, .stDownloadButton, .stFileUploader, .stButton button { display: none !important; }
          /* alléger les chips et décorations */
...
def render_print_header():
    ...
    st.markdown(
        f"""
        <div class="print-hide" style="margin-bottom:6px"></div>
        <div style="display:none" class="print-only">
          <h2 style="margin:0">Réservations — {apt_name}</h2>
...
def main():
    ...
    # — Sélecteur d'appartement (met à jour les chemins actifs)
    ...
    if changed:
        try:
            ...
            pass
        # pas de rerun imposé ici ; navigation fluide
