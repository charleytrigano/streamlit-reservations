...
# Purge douce du cache au chargement (compat 1.35+)
try:
...
def _to_bool_series(s) -> pd.Series:
    ...
    return ser.astype(str).str.strip().str.lower().isin(["true","1","oui","vrai","yes","y","t"])
...
def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    # Nuits
    mask_ok = pd.Series([isinstance(x, (date, datetime)) for x in df["date_arrivee"]], index=df.index) & \
              pd.Series([isinstance(x, (date, datetime)) for x in df["date_depart"]], index=df.index)
    try:
    ...
    if years_ser.dropna().empty:
        years_ser = da_all.dt.year
    if months_ser.dropna().empty:
        months_ser = da_all.dt.month
    df["AAAA"] = pd.to_numeric(pd.Series(years_ser, index=df.index), errors="coerce")
    df["MM"]   = pd.to_numeric(pd.Series(months_ser, index=df.index), errors="coerce")
...
def vue_reservations(df, palette):
    ...
    # Séries robustes
    years_ser  = pd.to_numeric(pd.Series(dfn.get("AAAA")), errors="coerce")
    months_ser = pd.to_numeric(pd.Series(dfn.get("MM")),   errors="coerce")

    # si vides -> recalc
    if years_ser.dropna().empty or months_ser.dropna().empty:
        da = pd.to_datetime(dfn["date_arrivee"], errors="coerce")
        if years_ser.dropna().empty:  years_ser  = da.dt.year
        if months_ser.dropna().empty: months_ser = da.dt.month
        dfn["AAAA"] = years_ser
        dfn["MM"]   = months_ser
    ...
    plats_unique  = sorted(
        dfn["plateforme"].astype(str).str.strip().replace({"nan":"","None":""}).replace("", pd.NA).dropna().unique().tolist()
    )

    years  = ["Toutes"] + years_unique
    months = ["Tous"]   + (months_unique if months_unique else list(range(1,13)))
    ...
    st.dataframe(data.loc[order], use_container_width=True)
...
def _copy_preview(label: str, payload: str, key: str):
    ...
    st.caption("Sélectionne le texte et copie-le (Ctrl/Cmd+C).")
...
def vue_sms(df, palette):
    ...
    if pre.empty:
        ...
        if pick:
            ...
            _copy_preview("Prévisualisation / Copier", msg, key=f"pre_{i}")
    ...
    if post.empty:
        ...
        if pick2:
            ...
            _copy_preview("Prévisualisation / Copier", msg2, key=f"post_{j}")
            c1, c2, c3 = st.columns(3)
...
def vue_google_sheet(df, palette):
    ...
    try:
        ...
        show_email = st.checkbox("Afficher les colonnes email (si présentes)", value=False)
        if not show_email:
