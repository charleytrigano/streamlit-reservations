# app.py — Villa Tobias (STABLE, calendrier coloré + légende, palette plateformes persistée)
...
def load_palette() -> dict:
    try:
        ...
        pal = {}
    # complétion avec défauts (sans écraser les personnalisations)
    out = DEFAULT_PALETTE.copy()
    out.update({k: v for k, v in pal.items() if isinstance(k, str) and isinstance(v, str) and v.startswith("#")})
    return out
...
# NOTE: pas d’expander imbriqué (erreurs Streamlit). Un bloc simple en barre latérale.
def palette_sidebar_editor(palette: dict) -> dict:
    st.sidebar.markdown("### 🎨 Plateformes")
    # affichage rapide existantes
    if palette:
        for pf, col in sorted(palette.items()):
            st.sidebar.markdown(
                f'<div style="display:flex;align-items:center;gap:8px;margin:2px 0">'
                f'<span style="display:inline-block;width:14px;height:14px;background:{col};border:1px solid #999"></span>'
                f'<span style="font-size:0.9rem">{pf}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
    st.sidebar.markdown("---")
    new_pf = st.sidebar.text_input("➕ Nouvelle/édition plateforme", "")
    new_color = st.sidebar.color_picker("Couleur", value="#888888", key="pf_color_picker")
    c1, c2 = st.sidebar.columns(2)
    if c1.button("Enregistrer plateforme"):
        pf = new_pf.strip()
        if pf:
            palette = dict(palette)  # copie
            palette[pf] = new_color
            save_palette(palette)
            st.sidebar.success(f"✅ Plateforme “{pf}” enregistrée")
            st.rerun()
    if c2.button("Réinitialiser défauts"):
        save_palette(DEFAULT_PALETTE.copy())
        st.sidebar.success("Palette par défaut restaurée")
        st.rerun()
    return palette
...
def bouton_telecharger(df: pd.DataFrame):
    ...
    st.sidebar.download_button(
        ...
        help="Utilisez ce bouton pour télécharger une copie de sécurité."
    )
...
def vue_reservations(df: pd.DataFrame):
    ...
    # Bloc d’options — pas d’expander imbriqué
    ...
    # Application du filtre payé (corrigé)
    if filtre_paye == "Payé":
    ...
    # Éditeur : éditables UNIQUEMENT paye & sms_envoye
    core_edit = core.copy()
...
def vue_ajouter(df: pd.DataFrame):
    ...
    # Plateforme libre : liste existante + possibilité de saisir autre chose
    pal = load_palette()
    options_pf = sorted(pal.keys())
    plateforme = inline_input("Plateforme", st.selectbox, key="add_pf",
                              options=options_pf, index=options_pf.index("Booking") if "Booking" in options_pf else 0)
...
def vue_modifier(df: pd.DataFrame):
    ...
    pal = load_palette()
    options_pf = sorted(pal.keys())
    pf_init = df.at[i,"plateforme"]
    pf_index = options_pf.index(pf_init) if pf_init in options_pf else (options_pf.index("Autre") if "Autre" in options_pf else 0)
...
def _calendar_legend_html(palette: dict, plateformes_presentes: list[str]) -> str:
    items = []
    for pf in plateformes_presentes:
        col = palette.get(pf, "#888888")
        items.append(
            f'<div style="display:flex;align-items:center;gap:8px;margin:2px 10px 2px 0">'
            f'<span style="display:inline-block;width:14px;height:14px;background:{col};border:1px solid #777"></span>'
            f'<span style="font-size:0.9rem">{pf}</span>'
            f'</div>'
        )
    return '<div style="display:flex;flex-wrap:wrap;align-items:center;margin-top:6px">' + "".join(items) + '</div>'
...
def vue_calendrier(df: pd.DataFrame):
    ...
    # Prépare le contenu de chaque jour (concat de noms clients)
    reservations_par_jour = {j: [] for j in jours}
    plateformes_presentes = set()
    ...
    for _, row in core.iterrows():
        ...
        nom = str(row.get("nom_client") or "")
        plateformes_presentes.add(pf)
        for j in jours:
            if d1 <= j < d2:
                reservations_par_jour[j].append((pf, nom))

    # Construit un tableau HTML coloré
    headers = ["Lun","Mar","Mer","Jeu","Ven","Sam","Dim"]
    cal = calendar.Calendar(firstweekday=0)  # lundi=0 ? (calendar module: 0=Monday)
    weeks = cal.monthdayscalendar(annee, mois_index)  # semaines avec jours (0 = vide)

    # Styles de base (fond clair lisible)
    st.markdown("""
    <style>
      .vt-cal { border-collapse: collapse; width: 100%; table-layout: fixed; }
      .vt-cal th, .vt-cal td { border: 1px solid #ddd; vertical-align: top; padding: 6px; }
      .vt-cal th { background: #f4f6f8; text-align: center; font-weight: 600; }
      .vt-daynum { font-size: 0.85rem; opacity: 0.8; margin-bottom: 6px; }
      .vt-badge { display:block; margin:3px 0; padding:4px 6px; border-radius:6px; border:1px solid rgba(0,0,0,0.15); color:#000; }
    </style>
    """, unsafe_allow_html=True)

    html = ['<table class="vt-cal">']
    # Header
    html.append("<tr>" + "".join(f"<th>{h}</th>" for h in headers) + "</tr>")

    # Body
    # Attention: monthdayscalendar renvoie semaines avec 7 entiers (jour ou 0)
    for week in weeks:
        html.append("<tr>")
        for day in week:
            if day == 0:
                html.append('<td style="background:#fafafa"></td>')
            else:
                d = date(annee, mois_index, day)
                badges = []
                # Chaque résa: badge plein couleur de la plateforme + nom du client
                for pf, nom in reservations_par_jour.get(d, []):
                    col = pal.get(pf, "#888888")
                    label = f"{nom}"
                    badges.append(f'<span class="vt-badge" style="background:{col}33;border-color:{col};">{label}</span>')
                cell = f'<div class="vt-daynum">{day}</div>' + "".join(badges)
                html.append(f"<td>{cell}</td>")
        html.append("</tr>")
    html.append("</table>")

    st.markdown("".join(html), unsafe_allow_html=True)
    if plateformes_presentes:
        st.markdown("**Légende**")
        st.markdown(_calendar_legend_html(pal, sorted(plateformes_presentes)), unsafe_allow_html=True)
    ...
    with colB:
        ...
        if dep_24h.empty:
            ...
            for idx, r in dep_24h.reset_index(drop=True).iterrows():
                ...
                st.code(body)
                c1, c2 = st.columns(2)
                if tel_link: c1.link_button(f"📞 Appeler {tel}", f"tel:{tel}")
                if sms_link: c2.link_button("📩 Envoyer SMS", f"sms:{tel}?&body={quote(body)}")
                st.divider()
...
def main():
    ...
    # Palette (éditeur en barre latérale, persistant)
    st.sidebar.markdown("---")
    palette_sidebar_editor(load_palette())
