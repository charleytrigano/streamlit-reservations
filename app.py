...
def ensure_schema(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Prix net
    brut = pd.to_numeric(df.get("prix_brut"), errors="coerce").fillna(0.0)
...
def vue_reservations(df, palette):
    ...
    # Filtres
    annees = ["Toutes"] + sorted(df["AAAA"].dropna().unique().astype(int).tolist(), reverse=True)
    ...
    base = float(data["base"].sum()) if "base" in data.columns else 0
    charges = float(data["charges"].sum()) if "charges" in data.columns else 0
    ...
    c5.metric("Base", f"{base:,.0f} €".replace(",", " "))
    c6.metric("Charges", f"{charges:,.0f} €".replace(",", " "))
