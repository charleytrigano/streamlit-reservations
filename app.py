# app.py — Villa Tobias (COMPLET) - Version avec Ajout fonctionnel
...
def sauvegarder_donnees_csv(df):
    ...
    try:
        df_to_save = df.copy()
        # S'assurer que les dates sont bien au format texte DD/MM/YYYY pour la sauvegarde
        for col in ['date_arrivee', 'date_depart']:
            if col in df_to_save.columns:
                df_to_save[col] = pd.to_datetime(df_to_save[col]).dt.strftime('%d/%m/%Y')
        
        df_to_save.to_csv(CSV_RESERVATIONS, sep=';', index=False)
        st.cache_data.clear() # Vider le cache pour forcer la relecture des données
        return True
    except Exception as e:
...
def ensure_schema(df):
    ...
    BASE_COLS = [
        'paye', 'nom_client', 'plateforme', 'date_arrivee', 'date_depart', 'nuitees',
