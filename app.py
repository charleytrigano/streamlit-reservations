# app.py — Villa Tobias (COMPLET, propre, 2 parties)
# - Réservations (édition payé/SMS + filtre payé + recherche + KPI)
# - Ajouter / Modifier / Supprimer
# - Calendrier coloré par plateforme (lisible en thème sombre)
# - Rapport / Clients / Export ICS / SMS
# - Restauration XLSX robuste (BytesIO + engine="openpyxl")
# - Vidage de cache
# NOTE: collez ensuite la Partie 2 à la suite de ce fichier.
...
def sms_message_arrivee(row: pd.Series) -> str:
    ...
    return (
        ...
        f"Telephone : {tel_aff}\n\n"
        "Bienvenue chez nous !\n\n "
        "Nous sommes ravis de vous accueillir bientot à Nice. Merci de nous indiquer votre heure d'arrivée.\n\n "
        "Une place de parking vous est allouée.\n\n "
        "Check-in à partir de 14h, check-out au plus tard 11h.\n\n "
        "Des consignes à bagages sont disponibles si besoin.\n\n "
    ...
    for semaine in monthcal:
        ...
        for jour in semaine:
            if jour == 0:
                ...
                # contenu: jour + liste des noms (max 5 lignes)
                if len(items) > 5:
