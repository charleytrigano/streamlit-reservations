# app.py — Villa Tobias (COMPLET, stable, fix "ambiguous DataFrame truth")
...
# - Calendrier mensuel coloré par plateforme + légende
# - KPI, exports, SMS
# - FIX: aucun test booléen ambigu sur DataFrame (ni "or df", ni "if df:")
...
def _extract_res_df(book: dict) -> pd.DataFrame:
    ...
    if isinstance(book, dict):
        if "reservations" in book and isinstance(book["reservations"], pd.DataFrame):
            df = book["reservations"]
        elif "Sheet1" in book and isinstance(book["Sheet1"], pd.DataFrame):
            df = book["Sheet1"]
...
def _extract_palette(book: dict) -> dict:
    ...
    if not isinstance(pal_df, pd.DataFrame) or pal_df.empty:
        return DEFAULT_PALETTE.copy()

    # Normaliser colonnes (sécurisé)
    cols_lower = {}
    for c in pal_df.columns:
        key = str(c).lower() if not pd.isna(c) else ""
        cols_lower[key] = c

    k_pf  = cols_lower.get("plateforme", None)
    if k_pf is None:
        k_pf = cols_lower.get("plateformes", None)
    k_col = cols_lower.get("couleur", None)
    if k_col is None:
        k_col = cols_lower.get("color", None)
    if k_pf is None or k_col is None:
        # fallback basique si 2 colonnes
        if pal_df.shape[1] >= 2:
            k_pf  = pal_df.columns[0]
            k_col = pal_df.columns[1]
        else:
            return DEFAULT_PALETTE.copy()

    palette = {}
    for _, r in pal_df.iterrows():
        name = str(r.get(k_pf, "")).strip()
        col = str(r.get(k_col, "")).strip()
...
def _palette_df_from_dict(palette: dict) -> pd.DataFrame:
    ...
    return pd.DataFrame({"plateforme": names, "couleur": cols})

def _get_palette() -> dict:
    pal = st.session_state.get("palette", None)
    if not isinstance(pal, dict) or len(pal) == 0:
    ...
    for k, v in pal.items():
        ks = str(k).strip()
        vs = str(v).strip()
        if ks and vs.startswith("#") and len(vs) in (4,7):
            out[ks] = vs
    if len(out) == 0:
...
def charger_donnees() -> pd.DataFrame:
    # Fichier absent -> df vide + palette défaut
...
def bouton_restaurer():
    ...
    if up is not None:
        try:
            ...
            bio = BytesIO(raw)
            book = pd.read_excel(bio, sheet_name=None, engine="openpyxl", converters={"telephone": normalize_tel})

            df_new = _extract_res_df(book)
            df_new = ensure_schema(df_new)

            pal_new = _extract_palette(book)
            st.session_state.palette = pal_new
...
def bouton_telecharger(df: pd.DataFrame):
    ...
    try:
        with pd.ExcelWriter(buf, engine="openpyxl") as w:
            ensure_schema(df).to_excel(w, index=False, sheet_name="reservations")
            pal_df.to_excel(w, index=False, sheet_name="plateformes")
    ...
    st.sidebar.download_button(
        ...
        help="Télécharge l’état actuel (réservations + plateformes)."
    )
...
def sms_message_depart(row: pd.Series) -> str:
    ...
    return (
        f"Bonjour {nom},\n\n"
        "Merci d’avoir choisi notre appartement ! Nous espérons que votre séjour s’est très bien passé.\n\n"
        "Si vous souhaitez revenir, nous serons ravis de vous accueillir à nouveau.\n\n"
        "Au plaisir,\n"
        "Annick & Charley"
    )
...
def vue_reservations(df: pd.DataFrame):
    ...
    with c2:
        st.info("Pour modifier ou supprimer une réservation, utilisez l’onglet **✏️ Modifier / Supprimer**.")
...
def vue_calendrier(df: pd.DataFrame):
    ...
    annees_vals = [int(x) for x in df["AAAA"].dropna().unique()]
    annees_vals.sort()
    if len(annees_vals) == 0:
        st.warning("Aucune année disponible.")
        return
    annee = cols[1].selectbox("Année", annees_vals, index=len(annees_vals)-1)
    ...
    st.caption("Légende :")
    leg = " • ".join([
        f'<span style="display:inline-block;width:0.9em;height:0.9em;background:{_get_palette()[p]};margin-right:6px;border-radius:3px;"></span>{p}'
        for p in sorted(_get_palette().keys())
    ])
...
def vue_rapport(df: pd.DataFrame):
    ...
    c1, c2, c3 = st.columns(3)
    annee = c1.selectbox("Année", annees_vals, index=len(annees_vals)-1, key="rapport_annee")
...
def vue_clients(df: pd.DataFrame):
    ...
    annees_vals = [int(x) for x in df["AAAA"].dropna().unique()]
    annees_vals.sort()
    annee = c1.selectbox("Année", annees_vals, index=len(annees_vals)-1) if len(annees_vals)>0 else None
