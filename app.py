...
# - Palette plateformes éditable (sidebar)
# - Calendrier mensuel : cases colorées par plateforme + noms clients
# - Filtre Payé corrigé
# - Bouton sauvegarde/restauration Excel
# - SMS (arrivées demain / relance +24h, et composeur manuel)
...
def vue_calendrier(df: pd.DataFrame):
    """Calendrier mensuel en tableau HTML : chaque case colorée par plateforme avec nom du client."""
    palette = get_palette()
    ...
    # Construire planning : jour -> [(plateforme, nom_client)]
    core, _ = split_totals(df)
    ...
    # HTML : <table> avec styles inline
    html = []
    html.append("""
    <style>
      .cal { border-collapse: collapse; width: 100%; table-layout: fixed; }
      .cal th, .cal td { border:1px solid rgba(127,127,127,0.25); vertical-align: top; }
      .cal th { padding:6px 4px; text-align:center; background:#fafafa; font-weight:600; }
      .day { min-height: 92px; padding:6px; font-size: 13px; line-height: 1.25; }
      .day .dnum { font-weight:700; margin-bottom:4px; opacity:0.8; }
      .dot { display:inline-block; width:0.7em; height:0.7em; border-radius:50%; margin-right:6px; vertical-align:-0.12em; }
      .name { display:block; margin:2px 0; }
    </style>
    """)
    ...
    for semaine in monthcal:
        ...
        for jour in semaine:
            if jour == 0:
                html.append('<td><div class="day"></div></td>')
                continue
            d = date(annee, mois_index, jour)
            items = planning.get(d, [])

            # Déterminer fond/texte + contenu
            if len(items) == 0:
                bg = "transparent"
                fg = "inherit"
                content = ""
                ...
                bg = lighten_color(base, 0.80)
                fg = ideal_text_color(bg)
                content = f'<span class="name">{nom}</span>'
            else:
                # Plusieurs réservations : fond neutre, chaque ligne avec pastille couleur plateforme
                bg = "#f7f7f7"
                fg = "#111111"
                lines = []
                for pf, nom in items:
                    c = palette.get(pf, "#999999")
                    lines.append(f'<span class="name"><span class="dot" style="background:{c}"></span>{nom}</span>')
                content = "".join(lines)

            html.append(
                f'<td style="background:{bg}; color:{fg}">'
                f'  <div class="day">'
                f'    <div class="dnum">{jour}</div>'
                f'    {content}'
                f'  </div>'
                f'</td>'
            )
    ...
    st.caption("Légende :")
    leg = " • ".join([
        f'<span class="dot" style="background:{get_palette()[p]}"></span>{p}'
