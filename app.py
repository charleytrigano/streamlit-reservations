...
# ============================== PART 1/5 — IMPORTS, CONFIG, STYLES ==============================
...
# --------------------------------- CONSTANTES ---------------------------------
# Ces chemins sont automatiquement remplacés quand on sélectionne un appartement
CSV_RESERVATIONS = "reservations.csv"
CSV_PLATEFORMES  = "plateformes.csv"
APARTMENTS_CSV   = "apartments.csv"
COUNTRIES_CSV    = "countries.csv"   # table optionnelle indicatifs → pays + drapeau
...
GOOGLE_FORM_VIEW = ("https://docs.google.com/forms/d/e/1FAIpQLScLiaqSAY3JYriYZIk9qP75YGUyP0sxF8pzmhbIQqsSEY0jpQ"
                    "/viewform?embedded=true")
GOOGLE_SHEET_EMBED_URL = "https://docs.google.com/spreadsheets/d/1ci-4i8dZWzixt0p5WPdB2D8ePCpNQDD0jjZf41KtYns/edit?usp=sharing"
GOOGLE_SHEET_PUBLISHED_CSV = ("https://docs.google.com/spreadsheets/d/e/2PACX-1vSMie1mawlXGJtqC7KL_gSgeC9e8jwOxcqMzC1HmxxU8FCrOxD0HX"
                              "l5APTO939__tu7EPh6aiXHnSnF/pub?output=csv")

# --------------------------------- STYLES / PRINT ---------------------------------
def apply_style(light: bool):
    """
    Applique un thème clair/sombre + styles calendriers + styles impression A4 paysage.
    Cette fonction est appelée de façon *safe* dans main() (fallback no-op si absent).
    """
    ...
    card_bg = "#ffffff" if light else "#0b0d12"
    ...
    st.markdown(
        ...
        <style>
          ...
          .cal-header {{
            ...
            font-weight:700; opacity:.85; margin:6px 0 8px;
          }}
          .cal-grid {{ display:grid; grid-template-columns: repeat(7, 1fr); gap:8px; }}
          .cal-cell {{
            border:1px solid {border}; border-radius:10px; min-height:110px; padding:8px;
            position:relative; overflow:hidden; background:{card_bg};
          }}
          ...
          @media print {{
            [data-testid="stSidebar"], header, footer, .stButton, .stRadio, .stDownloadButton {{ display:none !important; }}
            .print-hide {{ display:none !important; }}
            .print-header {{ display:block !important; margin-bottom:10px; }}
          }}
          .print-header {{ display:none; }}
          /* Colonnes techniques à masquer dans dataframes (si nom de col correspond) */
          .col-tech, .column-res-id, .column-ical-uid {{ display:none !important; }}
        </style>
        """,
...
def print_buttons(location: str = "main"):
    """Bouton Imprimer (JS) — compatible iOS/desktop. Location: 'main' ou 'sidebar'."""
    ...
    st.markdown(
        ...
        try {
          ...
          if (btn && btn.parentElement) { btn.parentElement.onclick = () => window.print(); }
        } catch (e) {}
        </script>
        """,
        unsafe_allow_html=True
...
def render_print_header(title_text: str):
    """En-tête qui apparaît uniquement à l'impression."""
    st.markdown(
        f"""
        <div class="print-header">
          <h2 style="margin:0">{escape(title_text)}</h2>
          <div style="opacity:.7;font-size:.9rem">Imprimé le {datetime.now().strftime("%d/%m/%Y %H:%M")}</div>
          <hr>
        </div>
        """,
        unsafe_allow_html=True
...
# ---- Base locale d’indicatifs (fallback) ----
...
# ---- countries.csv : lecture/écriture + helpers drapeau/indicatif ----
def _ensure_countries_csv_exists():
    """Crée un countries.csv par défaut si absent."""
    if not os.path.exists(COUNTRIES_CSV):
        demo = pd.DataFrame([
            {"indicatif":"33","pays":"France","drapeau":"🇫🇷"},
            {"indicatif":"44","pays":"Royaume-Uni","drapeau":"🇬🇧"},
            {"indicatif":"49","pays":"Allemagne","drapeau":"🇩🇪"},
            {"indicatif":"34","pays":"Espagne","drapeau":"🇪🇸"},
            {"indicatif":"39","pays":"Italie","drapeau":"🇮🇹"},
            {"indicatif":"41","pays":"Suisse","drapeau":"🇨🇭"},
        ])
        try:
            demo.to_csv(COUNTRIES_CSV, sep=";", index=False, encoding="utf-8", lineterminator="\n")
        except Exception:
            pass

@st.cache_data(show_spinner=False)
def _load_countries_table() -> pd.DataFrame:
    _ensure_countries_csv_exists()
    raw = _load_file_bytes(COUNTRIES_CSV)
    if raw is None:
    ...
    for col in ["indicatif","pays","drapeau"]:
...
def _save_countries_table(df: pd.DataFrame) -> bool:
    try:
        base = df.copy()
        base["indicatif"] = base["indicatif"].astype(str).str.replace(r"\D","", regex=True)
        base["pays"] = base["pays"].astype(str).strip()
        base["drapeau"] = base["drapeau"].astype(str).strip()
        base = base[base["indicatif"]!=""].drop_duplicates(subset=["indicatif"])
        base.to_csv(COUNTRIES_CSV, sep=";", index=False, encoding="utf-8", lineterminator="\n")
        st.cache_data.clear()
        return True
    except Exception as e:
        st.error(f"Erreur enregistrement countries.csv : {e}")
        return False

def _flag_for_country(country: str) -> str:
    """Retourne l'emoji du drapeau si présent dans countries.csv, sinon vide."""
    tbl = _load_countries_table()
    m = tbl[tbl["pays"].str.lower() == str(country or "").strip().lower()]
    if not m.empty:
        fl = str(m.iloc[0]["drapeau"]).strip()
        return fl if fl else ""
    return ""

def _phone_country(phone: str) -> str:
    """Résout le pays depuis l'indicatif. Utilise d'abord countries.csv puis la base fallback."""
    ...
    if p.startswith("+"):
        ...
        return "France"  # intuition FR
    else:
        p1 = p

    # 1) table countries.csv
    tbl = _load_countries_table()
    for k in sorted(tbl["indicatif"].unique(), key=lambda x: -len(str(x))):
        if str(p1).startswith(str(k)):
            row = tbl[tbl["indicatif"] == str(k)]
            if not row.empty:
                return str(row.iloc[0]["pays"]) or "Inconnu"
...
def vue_reservations(df: pd.DataFrame, palette: dict):
    ...
    st.markdown(
        (
            "<div class='glass'>"
            "<span class='chip'><small>Total brut</small><br><strong>{:.2f} €</strong></span>"
            "<span class='chip'><small>Total net</small><br><strong>{:.2f} €</strong></span>"
            "<span class='chip'><small>Charges</small><br><strong>{:.2f} €</strong></span>"
            "<span class='chip'><small>Base</small><br><strong>{:.2f} €</strong></span>"
            "<span class='chip'><small>Nuitées</small><br><strong>{}</strong></span>"
            "<span class='chip'><small>ADR (net)</small><br><strong>{:.2f} €</strong></span>"
            "</div>"
        ).format(brut, net, charges, base, nuits, adr).replace(",", " "),
        unsafe_allow_html=True
    )
    st.markdown("---")
    ...
    # masque colonnes techniques dans la grille
    hide_cols = ["ical_uid","%","charges","base"]
    show = [c for c in data.columns if c not in hide_cols]
...
def vue_plateformes(df: pd.DataFrame, palette: dict):
    ...
    # compatibilité versions Streamlit (ColorColumn pas toujours dispo)
...
# ============================== PART 4/5 — VUES CALENDRIER & RAPPORT ==============================
...
def vue_calendrier(df: pd.DataFrame, palette: dict):
    ...
    # petite fonction d’échappement pour attribut HTML
    def _esc_attr(s) -> str:
            ...
            if not outside:
                ...
                if not rs.empty:
                    for _, r in rs.iterrows():
                        ...
                        title_txt = _esc_attr(r.get("nom_client", ""))
                        cell += (
                            "<div class='resa-pill' "
...
def vue_rapport(df: pd.DataFrame, palette: dict):
    ...
    st.markdown(
        (
            "<div class='glass'>"
            "<span class='chip'><small>Taux global</small><br><strong>{:.1f}%</strong></span>"
            "<span class='chip'><small>Nuitées occupées</small><br><strong>{}</strong></span>"
            "<span class='chip'><small>Jours dispos</small><br><strong>{}</strong></span>"
            "<span class='chip'><small>Pays filtré</small><br><strong>{}</strong></span>"
            "</div>"
        ).format(taux_global_filtered, int(filtered_nuitees), int(filtered_jours), (payf if payf!='Tous' else 'Tous')),
        unsafe_allow_html=True
    )
    ...
    def _jours_dans_annee(annee): return 366 if (annee%4==0 and annee%100!=0) or (annee%400==0) else 365
    ...
    st.markdown(
        (
            "<div class='glass'>"
            "<span class='chip'><small>Année</small><br><strong>{}</strong></span>"
            "<span class='chip'><small>Pays distincts</small><br><strong>{}</strong></span>"
            "<span class='chip'><small>Total réservations</small><br><strong>{}</strong></span>"
            "<span class='chip'><small>Top pays (CA net)</small><br><strong>{}</strong></span>"
            "</div>"
        ).format(year_pays, nb_pays, total_res, top_pays),
        unsafe_allow_html=True
    )
    disp = agg_pays.copy()
...
def vue_sms(df: pd.DataFrame, palette: dict):
    ...
    if pre.empty:
        ...
        options = [f"{i}: {r['nom_client']} ({r['telephone']})" for i, r in pre.reset_index().iterrows()]
    ...
    if post.empty:
        ...
        options2 = [f"{i}: {r['nom_client']} — départ {r['date_depart']}" for i, r in post.reset_index().iterrows()]
