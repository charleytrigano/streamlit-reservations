...
def _select_apartment_sidebar():
    ...
    Change les chemins CSV_RESERVATIONS / CSV_PLATEFORMES à la volée.
    """
    ...
    # si changement d'appartement → maj session + chemins + clear cache
    changed = (
    ...
    # synchronise les constantes utilisées par le reste de l'app
    ...
    if changed:
        # vider le cache des @st.cache_data pour recharger les bons fichiers
        try:
            st.cache_data.clear()
        except Exception:
            pass
    ...
    # Bouton impression globale (affiché partout)
...
def vue_settings(df: pd.DataFrame, palette: dict):
    ...
    c1.download_button(
        ...
        mime="text/csv",
        key="dl_res_csv",
        help="Export du fichier de travail de l'appartement courant."
    )
    try:
    ...
    # ---------- Import manuel (remplacement immédiat) ----------
    ...
    # ---------- Outil secours : écraser apartments.csv ----------
...
# ============================== MAIN ==============================
def main():
    # query param clear=1 pour vider le cache
    ...
    # Sélecteur d'appartement AVANT tout le reste
    ...
    # Titre principal + nom appartement
    st.title("✨ Villa Tobias — Gestion des Réservations")
    st.markdown(f"### 🏷️ {apt_name}")

    # Charger données & palette de l'appartement courant
