...
def _apply_custom_css():
    # Print A4 paysage + petite mise à jour UI
    st.markdown(
        """
        <style>
          @media print {
...
def vue_sms(df, palette):
    ...
    if pre.empty:
        ...
        if pick:
            ...
            link_form = _google_form_prefill(r.get("res_id"), r.get("nom_client"), _format_phone_e164(r.get("telephone")),
...
# ============================== VUE : EXPORT ICS (avec “nouvelles”) ==============================
import json

def _ics_state_path() -> str:
    slug = st.session_state.get("apt_slug", "default")
    return f".ics_state_{slug}.json"

def _load_ics_state() -> dict:
    path = _ics_state_path()
...
def _save_ics_state(state: dict):
    path = _ics_state_path()
    try:
        with open(path, "w", encoding="utf-8") as f:
            json.dump(state, f, ensure_ascii=False, indent=2)
    except Exception as e:
        st.warning(f"Impossible d'enregistrer l'état ICS : {e}")
...
def vue_export_ics(df, palette):
    ...
    # État persistant des UID déjà exportés pour cet appartement
    state = _load_ics_state()
    already = set(state.get("exported_uids", []))
    ...
    plats = ["Tous"] + sorted(dfa["plateforme"].dropna().astype(str).unique())
    plat  = st.selectbox("Plateforme", plats, index=0)
    new_only = st.checkbox("Nouvelles depuis la dernière export (par UID)", value=True)
    ...
    # Filtre "nouvelles"
    if new_only:
        data = data[~data["ical_uid"].isin(already)]

    st.caption(f"Réservations à exporter : **{len(data)}**")
    st.dataframe(
        data[["nom_client","plateforme","date_arrivee","date_depart","nuitees","prix_brut","res_id","ical_uid"]],
        use_container_width=True
    )

    if data.empty:
        st.warning("Aucune réservation à exporter selon les filtres.")
        return
    ...
    st.download_button(
        ...
        file_name=f"reservations_{year}{'__new' if new_only else ''}.ics",
        mime="text/calendar",
        key="dl_ics_btn"
    )

    # Marquer comme exportées (mise à jour de l'état persistant)
    if st.button("✅ Marquer ces réservations comme exportées (UID)", key="mark_exported_btn"):
        exported_now = set(data["ical_uid"].astype(str).tolist())
        merged = sorted(set(already).union(exported_now))
        state["exported_uids"] = merged
        state["last_export"] = datetime.utcnow().isoformat()
        _save_ics_state(state)
        st.success(f"{len(exported_now)} UID ajoutés à l'état 'exportés'.")
        st.cache_data.clear()
...
def vue_settings(df: pd.DataFrame, palette: dict):
    ...
    print_buttons()
    st.caption("Sauvegarde, restauration, cache, diagnostic et écrasement apartments.csv.")
        ...
        try:
            ...
            if st.button("✅ Confirmer la restauration", key="confirm_restore_settings"):
                try:
                    ...
                    save.to_csv(st.session_state.get("CSV_RESERVATIONS", CSV_RESERVATIONS),
    ...
    # Diagnostic appartements
    st.markdown("### 🔎 Diagnostics")
    try:
        apdf = _read_apartments_csv()
        st.write("**Apartments détectés :**")
        st.dataframe(apdf, use_container_width=True)
        st.code(f"CSV actif: {st.session_state.get('CSV_RESERVATIONS','?')} | Palette active: {st.session_state.get('CSV_PLATEFORMES','?')}")
    except Exception as e:
        st.warning(f"Diag apts: {e}")
