...
# --- HARD CLEAR (purge cache au chargement) ---
try:
...
def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        pct = np.where(_to_num(df["prix_brut"])>0, (_to_num(df["charges"]) / _to_num(df["prix_brut"]) * 100), 0.0)
    ...
    if miss_uid.any():
        df.loc[miss_uid,"ical_uid"] = df.loc[miss_uid].apply(lambda r: hashlib.sha1(
            f"{r.get('res_id','')}{r.get('nom_client','')}{r.get('telephone','')}".encode()
        ).hexdigest()+"@villa-tobias", axis=1)
...
# Valeurs de secours si non définies en Partie 1
FORM_SHORT_URL = globals().get("FORM_SHORT_URL", "https://urlr.me/kZuH94")
GOOGLE_FORM_URL = globals().get("GOOGLE_FORM_URL", "https://docs.google.com/forms/d/e/1FAIpQLScLiaqSAY3JYriYZIk9qP75YGUyP0sxF8pzmhbIQqsSEY0jpQ/viewform")
GOOGLE_SHEET_EMBED_URL = globals().get("GOOGLE_SHEET_EMBED_URL", "https://docs.google.com/spreadsheets/d/1ci-4i8dZWzixt0p5WPdB2D8ePCpNQDD0jjZf41KtYns/edit?usp=sharing")
GOOGLE_SHEET_PUBLISHED_CSV = globals().get("GOOGLE_SHEET_PUBLISHED_CSV", "https://docs.google.com/spreadsheets/d/e/2PACX-1vSMie1mawlXGJtqC7KL_gSgeC9e8jwOxcqMzC1HmxxU8FCrOxD0HXl5APTO939__tu7EPh6aiXHnSnF/pub?output=csv")
...
def vue_calendrier(df, palette):
    ...
    st.markdown("<div style='display:grid;grid-template-columns:repeat(7,1fr);font-weight:700;opacity:.8;margin-top:10px'>"
                "<div>Lun</div><div>Mar</div><div>Mer</div><div>Jeu</div><div>Ven</div><div>Sam</div><div>Dim</div></div>", unsafe_allow_html=True)
...
def _safe_bool_series(s):
    """Force une série booléenne (False si manquant). Évite les erreurs avec ~."""
    if s is None:
        return pd.Series([], dtype=bool)
    if s.dtype == bool:
...
def vue_sms(df, palette):
    ...
    pre["sms_envoye"]   = _safe_bool_series(pre["sms_envoye"])
    ...
    if pre.empty:
        ...
        if pick:
            ...
            st.text_area("Message (copier/coller)", value=msg, height=220)
    ...
    post["post_depart_envoye"] = _safe_bool_series(post["post_depart_envoye"])
    ...
    if post.empty:
        ...
        if pick2:
            ...
            st.text_area("Message (copier/coller)", value=msg2, height=200, key=f"ta_post_{j}")
            c1, c2, c3 = st.columns(3)
...
def vue_export_ics(df, palette):
    ...
    years = sorted(pd.to_numeric(df["AAAA"], errors="coerce").dropna().astype(int).unique(), reverse=True)
    if not years: 
        st.warning("Pas d'année détectée."); return
    ...
    if miss.any():
        data.loc[miss, "ical_uid"] = data[miss].apply(lambda r: hashlib.sha1(
            f"{r.get('res_id','')}{r.get('nom_client','')}{r.get('telephone','')}".encode()
        ).hexdigest()+"@villa-tobias", axis=1)
...
def admin_sidebar(df):
    ...
    if up is not None and st.sidebar.button("Confirmer restauration"):
        try:
            content = up.getvalue()  # important: ne pas réutiliser .read() plusieurs fois
