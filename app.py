...
# ============================== 1. CACHE BUSTING (robuste) ==============================
def get_cache_salt() -> int:
    if "cache_salt" not in st.session_state:
        st.session_state["cache_salt"] = 0
    return st.session_state["cache_salt"]

def bump_cache_salt():
    st.session_state["cache_salt"] = int(st.session_state.get("cache_salt", 0)) + 1
...
def apply_style(light: bool):
    ...
    st.markdown(
        ...
        <style>
          ...
          .small-note {{ font-size:0.85rem; opacity:.8 }}
        </style>
        """,
...
def _detect_delimiter_and_read(raw: bytes) -> pd.DataFrame:
    """Lecture CSV tolérante : ; , tab |  -> dtype=str ; strip colonnes."""
    if raw is None:
        return pd.DataFrame()
...
def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    """Normalise pour respecter BASE_COLS + champs dérivés (robuste)."""
...
@st.cache_data
def _load_file_bytes(path: str, salt: int):
    try:
...
@st.cache_data
def charger_donnees(salt: int):
    # RESA
    raw = _load_file_bytes(CSV_RESERVATIONS, salt)
    base_df = _detect_delimiter_and_read(raw) if raw is not None else pd.DataFrame()
    df = ensure_schema(base_df)

    # PALETTE
    rawp = _load_file_bytes(CSV_PLATEFORMES, salt)
...
def sauvegarder_donnees(df: pd.DataFrame) -> bool:
    try:
        ...
        bump_cache_salt()  # invalide le cache FIABLEMENT
        return True
...
def vue_google_sheet(df, palette):
    ...
    try:
        ...
        st.caption("Astuce : on peut fusionner automatiquement l’email au client à partir du res_id si vous le collectez dans le formulaire.")
    except Exception as e:
...
def admin_sidebar(df):
    ...
    if st.sidebar.button("🧹 Vider le cache & recharger"):
        bump_cache_salt()
        st.success("Cache invalidé. Rechargement…")
        st.rerun()
...
def main():
    ...
    # Purge via URL (optionnel): ?purge=1
    try:
        q = _get_query_params()
        if str(q.get("purge", ["0"])[0]) in ("1", "true", "True"):
            bump_cache_salt()
            st.success("Cache invalidé via URL. Rechargement…")
    ...
    salt = get_cache_salt()
    df, palette_loaded = charger_donnees(salt)
