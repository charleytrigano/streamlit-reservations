...
# - Onglet Plateformes (ajout / modif / suppression + persistance dans feuille Excel "Plateformes")
# - Réservations (liste + cases Payé/SMS éditables)
# - Ajouter / Modifier / Supprimer
# - Calendrier mensuel : cases colorées par plateforme + noms clients
# - Rapport (détail + KPI + graphiques)
# - Clients
# - Export ICS
# - SMS
# - Sauvegarde & Restauration Excel (openpyxl) avec anti-boucle
...
SHEET_RES = "Sheet1"         # Feuille des réservations
SHEET_PLAT = "Plateformes"   # Feuille des plateformes
...
def _palette_clean(pal: dict) -> dict:
    out = {}
...
def _df_palette_to_dict(dfp: pd.DataFrame) -> dict:
    pal = {}
    if dfp is not None and not dfp.empty:
        for _, r in dfp.iterrows():
            ...
            if k and v.startswith("#"):
                pal[k] = v
    return _palette_clean(pal)

def _palette_dict_to_df(pal: dict) -> pd.DataFrame:
    pal = _palette_clean(pal)
    return pd.DataFrame({"plateforme": list(pal.keys()), "couleur": list(pal.values())})

def get_palette() -> dict:
    if "palette" not in st.session_state:
        st.session_state.palette = DEFAULT_PALETTE.copy()
    st.session_state.palette = _palette_clean(st.session_state.palette)
    return st.session_state.palette

def set_palette(pal: dict):
    st.session_state.palette = _palette_clean(pal)
...
def _read_excel_cached(path: str, mtime: float):
    # Renvoie (df_reservations, df_plateformes)
    try:
        x = pd.ExcelFile(path, engine="openpyxl")
        df_res = pd.read_excel(x, sheet_name=SHEET_RES, engine="openpyxl",
                               converters={"telephone": normalize_tel})
        try:
            df_pf = pd.read_excel(x, sheet_name=SHEET_PLAT, engine="openpyxl")
        except Exception:
...
def charger_workbook():
    if not os.path.exists(FICHIER):
        ...
        df_pf = _palette_dict_to_df(DEFAULT_PALETTE)
        return df_res, df_pf
    try:
        mtime = os.path.getmtime(FICHIER)
        df_res, df_pf = _read_excel_cached(FICHIER, mtime)
        df_res = ensure_schema(df_res)
        return df_res, df_pf
    except Exception as e:
        st.error(f"Erreur de lecture Excel : {e}")
        return ensure_schema(pd.DataFrame()), _palette_dict_to_df(DEFAULT_PALETTE)

def sauvegarder_workbook(df_res: pd.DataFrame, df_pf: pd.DataFrame):
    df_res = ensure_schema(df_res)
    core, totals = split_totals(df_res)
    core = sort_core(core)
    out = pd.concat([core, totals], ignore_index=True)

    # Sécuriser df_pf
    if df_pf is None or df_pf.empty or "plateforme" not in df_pf.columns or "couleur" not in df_pf.columns:
        df_pf = _palette_dict_to_df(get_palette())
...
def bouton_restaurer():
    up = st.sidebar.file_uploader(
        ...
        help="Remplace le fichier actuel (2 feuilles : Sheet1 et Plateformes)"
    )
    if up is not None and st.sidebar.button("Restaurer maintenant"):
        try:
            ...
            try:
                ...
                df_pf = _palette_dict_to_df(DEFAULT_PALETTE)
            df_res = ensure_schema(df_res)
            set_palette(_df_palette_to_dict(df_pf))
            sauvegarder_workbook(df_res, df_pf)
...
def bouton_telecharger(df_res: pd.DataFrame, df_pf: pd.DataFrame):
    buf = BytesIO()
    try:
        with pd.ExcelWriter(buf, engine="openpyxl") as w:
            ensure_schema(df_res).to_excel(w, index=False, sheet_name=SHEET_RES)
            (df_pf if df_pf is not None else _palette_dict_to_df(get_palette())).to_excel(
                w, index=False, sheet_name=SHEET_PLAT
            )
        st.sidebar.download_button(
            ...
            help="Télécharge une copie (2 feuilles)."
        )
    except Exception as e:
...
# ==============================  VUES  ==============================
def vue_plateformes(df_res: pd.DataFrame, df_pf: pd.DataFrame):
    st.title("🎨 Plateformes")
    pal = _df_palette_to_dict(df_pf) if df_pf is not None else get_palette()
    set_palette(pal)

    st.caption("Ajouter ou modifier une plateforme")
    c1, c2 = st.columns([2,1])
    with c1:
        nom_pf = st.text_input("Nom de la plateforme", key="pf_nom", placeholder="Ex: Expedia")
    with c2:
        col_pf = st.color_picker("Couleur", key="pf_color", value="#9b59b6")
    colA, colB = st.columns(2)
    if colA.button("Ajouter / Mettre à jour"):
        if not (nom_pf or "").strip():
            st.warning("Entrez un nom.")
        else:
            pal[(nom_pf or "").strip()] = col_pf
            set_palette(pal)
            st.success(f"✅ « {nom_pf} » mise à jour en mémoire.")
    if colB.button("Réinitialiser défauts"):
        pal = DEFAULT_PALETTE.copy()
        set_palette(pal)
        st.success("✅ Palette par défaut rechargée.")
    ...
    if pal:
        ...
        for k in sorted(pal.keys()):
            rows.append({"plateforme": k, "couleur": pal[k]})
        st.dataframe(pd.DataFrame(rows), use_container_width=True)

    st.markdown("---")
    if st.button("💾 Enregistrer définitivement dans Excel"):
        # On écrit dans la feuille Plateformes
        df_pf_new = _palette_dict_to_df(get_palette())
        sauvegarder_workbook(df_res, df_pf_new)
...
def vue_reservations(df: pd.DataFrame):
    ...
    if c1.button("💾 Enregistrer les cases cochées"):
        ...
        return new_df  # renvoyé au main() pour sauvegarde
...
def vue_calendrier(df: pd.DataFrame):
    ...
    pal = get_palette()
    leg = " • ".join([
        f'<span style="display:inline-block;width:0.9em;height:0.9em;background:{pal[p]};margin-right:6px;border-radius:3px;"></span>{p}'
...
# ==============================  APP  ==============================
def main():
    # Charger le classeur (2 feuilles)
    df_res, df_pf = charger_workbook()
    # Synchroniser la palette en session depuis la feuille Plateformes
    set_palette(_df_palette_to_dict(df_pf))

    # Sidebar : Fichier & Maintenance
    st.sidebar.title("📁 Fichier")
    bouton_telecharger(df_res, df_pf)
    bouton_restaurer()
    st.sidebar.markdown("---")
    ...
    onglet = st.sidebar.radio(
        "Aller à",
        ["🎨 Plateformes","📋 Réservations","➕ Ajouter","✏️ Modifier / Supprimer",
         "📅 Calendrier","📊 Rapport","👥 Liste clients","📤 Export ICS","✉️ SMS"]
    )

    # Router
    if onglet == "🎨 Plateformes":
        vue_plateformes(df_res, df_pf)

    elif onglet == "📋 Réservations":
        maybe_df = vue_reservations(df_res)
        if isinstance(maybe_df, pd.DataFrame):
            # sauvegarder
            sauvegarder_workbook(maybe_df, _palette_dict_to_df(get_palette()))
            st.experimental_rerun()

    elif onglet == "➕ Ajouter":
        maybe_df = vue_ajouter(df_res)
        if isinstance(maybe_df, pd.DataFrame):
            sauvegarder_workbook(maybe_df, _palette_dict_to_df(get_palette()))
