...
# - 🔧 Plateformes : page dédiée (ajout / modif / suppr) + sauvegarde Excel (feuille "Plateformes")
# - Palette chargée depuis Excel si présent, sinon valeurs par défaut
# - Réservations / Ajouter / Modifier-Supprimer / Calendrier coloré / Rapport / Clients / Export ICS / SMS
# - Import/Export XLSX robustes (engine="openpyxl", BytesIO pour restauration)
# - Pas d’expander imbriqués
...
def get_palette_from_excel(path: str) -> dict:
    """Lit la feuille Plateformes si elle existe. Sinon renvoie DEFAULT_PALETTE."""
    ...
    try:
        with pd.ExcelFile(path, engine="openpyxl") as xls:
            if "Plateformes" in xls.sheet_names:
                ...
                if not {"plateforme","couleur"}.issubset(set(dfp.columns)):
                    return DEFAULT_PALETTE.copy()
                pal = {}
                for _, r in dfp.iterrows():
                    name = str(r.get("plateforme") or "").strip()
                    col  = _normalize_hex(r.get("couleur") or "")
                    if name:
                        pal[name] = col
...
def save_palette_to_excel(path: str, palette: dict):
    """Écrit/Met à jour la feuille Plateformes (2 colonnes). Préserve la feuille principale."""
    # Charger fichier existant (ou créer DataFrame vide)
    core_df = None
    if os.path.exists(path):
        try:
            core_df = pd.read_excel(path, engine="openpyxl")
        except Exception:
            core_df = None

    # Construire la table Plateformes
    dfp = pd.DataFrame(
        [{"plateforme": k, "couleur": _normalize_hex(v)} for k, v in palette.items()],
        columns=["plateforme","couleur"]
    )

    # Écrire avec 2 feuilles : Sheet1 (réservations) + Plateformes
    with pd.ExcelWriter(path, engine="openpyxl") as w:
        if isinstance(core_df, pd.DataFrame):
            core_df.to_excel(w, index=False, sheet_name="Sheet1")
        else:
            # Si aucune feuille principale détectée, on crée une feuille vide conforme au schéma
            ensure_schema(pd.DataFrame()).to_excel(w, index=False, sheet_name="Sheet1")
        dfp.to_excel(w, index=False, sheet_name="Plateformes")

def get_palette() -> dict:
    """Source de vérité : Excel ; fallback session (SI défini) ; sinon défaut."""
    pal_excel = get_palette_from_excel(FICHIER)
    # Session peut contenir une version en cours d'édition (non sauvée) ; on l’utilise pour l’affichage live
    if "palette_edit" in st.session_state and isinstance(st.session_state.palette_edit, dict):
        # garantir des hex corrects
        tmp = {}
        for k,v in st.session_state.palette_edit.items():
            if k:
                tmp[str(k)] = _normalize_hex(v)
        return tmp if tmp else pal_excel
    return pal_excel

def set_palette_for_session(palette: dict):
    st.session_state.palette_edit = {str(k): _normalize_hex(v) for k,v in palette.items() if str(k)}
...
def sauvegarder_donnees(df: pd.DataFrame):
    ...
    try:
        with pd.ExcelWriter(FICHIER, engine="openpyxl") as w:
            ...
            # Conserver/écrire la feuille Plateformes actuelle
            pal = get_palette_from_excel(FICHIER)
            pd.DataFrame(
...
def bouton_restaurer():
    ...
    if up is not None:
        try:
            ...
            # Écrire seulement la Sheet1 (réservations). La feuille Plateformes sera conservée si le fichier contient aussi cette feuille.
            with pd.ExcelWriter(FICHIER, engine="openpyxl") as w:
                df_new.to_excel(w, index=False, sheet_name="Sheet1")
                # si l’upload contenait Plateformes, on tente de la copier aussi
                try:
                    bio.seek(0)
                    with pd.ExcelFile(bio, engine="openpyxl") as xls:
                        if "Plateformes" in xls.sheet_names:
                            ...
                            # sinon on réutilise ou recrée à partir de l’actuel
                            pd.DataFrame(
                                [{"plateforme": k, "couleur": v} for k, v in get_palette_from_excel(FICHIER).items()],
                                columns=["plateforme","couleur"]
                            ).to_excel(w, index=False, sheet_name="Plateformes")
                except Exception:
                    # fallback : palette par défaut
                    pd.DataFrame(
...
def sms_message_arrivee(row: pd.Series) -> str:
    ...
    return (
        ...
        f"Telephone : {tel_aff}\n\n"
        "Bienvenue chez nous !\n\n "
        "Pour organiser au mieux votre réception, merci de nous indiquer votre heure d'arrivée.\n\n "
...
def vue_ajouter(df: pd.DataFrame):
    ...
    pf_options = sorted(palette.keys()) or ["Autre"]
...
def vue_modifier(df: pd.DataFrame):
    ...
    palette = get_palette()
    options_pf = sorted(palette.keys()) or ["Autre"]
...
def vue_calendrier(df: pd.DataFrame):
    ...
    for semaine in monthcal:
        ...
        for jour in semaine:
            if jour == 0:
                ...
                content_lines = [str(jour)] + [f"{nom}" for _, nom in items[:6]]
                if len(items) > 6:
                    content_lines += [f"... (+{len(items)-6})"]
                row_text.append("\n".join(content_lines))
    ...
    st.caption("Légende :")
    leg = " • ".join([
        f'<span style="display:inline-block;width:0.9em;height:0.9em;background:{get_palette().get(p,"#999")};margin-right:6px;border-radius:3px;"></span>{p}'
...
def vue_plateformes():
    st.title("🔧 Plateformes (couleurs & sauvegarde)")

    # Charger palette actuelle (Excel ou défaut)
    pal = get_palette()
    if "palette_edit" not in st.session_state:
        set_palette_for_session(pal)

    # Liste éditable
    st.markdown("### Liste")
    dfp = pd.DataFrame(
        [{"plateforme": k, "couleur": v} for k, v in st.session_state.palette_edit.items()],
        columns=["plateforme","couleur"]
    )
    edited = st.data_editor(
        dfp,
    ...
    # Actions
    c1, c2, c3 = st.columns(3)
    if c1.button("💾 Enregistrer dans Excel"):
        ...
        for _, r in edited.iterrows():
            ...
            if name:
                new_pal[name] = col
        if not new_pal:
            st.error("Aucune plateforme valide (nom requis).")
        else:
            save_palette_to_excel(FICHIER, new_pal)
            set_palette_for_session(new_pal)
            st.success("✅ Palette enregistrée dans reservations.xlsx (feuille « Plateformes »).")
            st.rerun()

    if c2.button("↩️ Recharger depuis Excel"):
        set_palette_for_session(get_palette_from_excel(FICHIER))
        st.info("Palette rechargée depuis Excel.")
        st.rerun()

    if c3.button("🧹 Réinitialiser (défaut)"):
        set_palette_for_session(DEFAULT_PALETTE.copy())
        st.warning("Palette réinitialisée (non sauvée). Cliquez « Enregistrer dans Excel » pour la rendre définitive.")

    st.caption("Astuce : codes valides au format **#RRGGBB** (ex. #1e90ff).")
