...
# ------------------------------ CONSTANTES ------------------------------
# Ces chemins seront remplacés par l'appartement actif (voir PART 2)
...
def render_print_header():
    st.markdown(
        f"<div class='print-header'><h2 style='margin:0'>Export — {escape(st.session_state.get('apt_name','—'))}</h2></div>",
        unsafe_allow_html=True
    )
...
def create_indicatifs_csv():
    ...
    if not os.path.exists(INDICATIFS_CSV):
        try:
            ...
            st.warning(f"Impossible d’écrire {INDICATIFS_CSV} : {e}")

@st.cache_data(show_spinner=False)
def load_indicatifs() -> pd.DataFrame:
    """Charge le CSV indicatifs (et le crée au besoin)."""
...
def _phone_country(phone: str) -> str:
    """Déduit le pays à partir du téléphone en utilisant INDICATIFS_CSV."""
...
# ============================== PART 2/5 — NORMALISATION & APARTMENTS ==============================

# ---------- Normalisation & sauvegarde ----------

def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    """Garantit la présence et le type des colonnes attendues, recalcule prix_net/base/%/nuitees, etc."""
...
def sauvegarder_donnees(df: pd.DataFrame) -> bool:
    """Sauvegarde le CSV des réservations pour l’appartement actif et vide le cache."""
    try:
...
def charger_donnees(csv_reservations: str, csv_plateformes: str):
    """Charge les données réservations + palette (et crée les fichiers s’ils n’existent pas)."""
    # Création fichiers par défaut si absents
    defaults = [
...
def _current_apartment() -> dict | None:
    """Retourne {'slug','name'} de l’appartement courant stocké en session, sinon None."""
...
def _select_apartment_sidebar() -> bool:
    ...
    Met à jour les chemins CSV_RESERVATIONS / CSV_PLATEFORMES en session.
    ...
    # met à jour les globales (nécessaire pour certaines fonctions utilitaires)
...
def _load_data_for_active_apartment():
    """Charge df & palette en tenant compte de l’appartement actif (variables session)."""
...
def vue_reservations(df: pd.DataFrame, palette: dict):
    ...
    st.markdown(
        (
            ...
            f"<span class='chip'><small>Charges</small><br><strong>{charges:,.2f} €</strong></span>"
            f"<span class='chip'><small>Base</small><br><strong>{base:,.2f} €</strong></span>"
            f"<span class='chip'><small>Nuitées</small><br><strong>{nuits}</strong></span>"
            f"<span class='chip'><small>ADR (net)</small><br><strong>{adr:,.2f} €</strong></span>"
            "</div>"
        ).replace(",", " "),
        unsafe_allow_html=True
    )
...
def vue_rapport(df: pd.DataFrame, palette: dict):
    ...
    # Déduire pays si absent via téléphone
