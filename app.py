...
# Chemins par défaut (seront remplacés après choix de l’appartement)
...
def print_buttons(location: str = "page"):
    ...
    st.markdown(
        ...
        window.addEventListener('load', () => {
          const btns = parent.document.querySelectorAll('button span');
          for (const s of btns) {
            if (s.innerText.trim() === "🖨️ Imprimer") {
              s.parentElement.onclick = () => window.print();
            }
          }
        });
        </script>
...
def build_stable_uid(row) -> str:
    ...
    return hashlib.sha1(base.encode()).hexdigest() + "@multi-apt"
...
def ensure_schema(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in BASE_COLS:
        if c not in df.columns: df[c] = pd.Series([None]*len(df), index=df.index)
    for c in df.columns: df[c] = _as_series(df[c], index=df.index)
...
# --------- Multi-appartements (sans mot de passe) ---------
...
def _current_apartment() -> dict | None:
    ...
    if slug and name: return {"slug": slug, "name": name}
    return None
...
def _select_apartment_sidebar() -> bool:
    ...
    try: print_buttons(location="sidebar")
    except Exception: pass
...
def vue_plateformes(df, palette):
    ...
    plats_df = sorted(
        df.get("plateforme", pd.Series([], dtype=str)).astype(str).str.strip().replace({"nan": ""}).dropna().unique().tolist()
    )
    ...
    if HAS_COLORCOL:
        ...
        col_cfg = {
            ...
            "couleur": st.column_config.TextColumn(
                "Couleur (hex)", help="Ex: #1e90ff", validate=r"^#([0-9A-Fa-f]{6})$", width="small"
            ),
        }
...
def vue_rapport(df, palette):
    ...
    def jours_dans_mois(periode_str):
        annee, m = map(int, periode_str.split("-"))
        return monthrange(annee, m)[1]
    ...
    col_export.download_button("⬇️ Exporter (CSV)", data=csv_occ, file_name="taux_occupation.csv", mime="text/csv")
    xlsx_occ, _ = _df_to_xlsx_bytes(occ_export, "Taux d'occupation")
    if xlsx_occ:
        col_export.download_button(
            "⬇️ Exporter (Excel)", data=xlsx_occ, file_name="taux_occupation.xlsx",
    ...
    for c in num_cols: disp[c] = pd.to_numeric(disp[c], errors="coerce")
